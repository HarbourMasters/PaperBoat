...
// PORT: uses gDPReadFB to capture the GPU framebuffer to CPU memory on demand,
// replacing N64's nuGfxCfb cycling. The captured buffer is used as a texture to
// show the frozen battle scene behind UI overlays (battle menu, dizzy dial, etc.)
extern u16* GetPrevFramePixels(void);
extern void RequestPrevFrameCapture(void);
...
void appendGfx_draw_prev_frame_buffer(s32 x1, s32 y1, s32 x2, s32 y2, f32 alpha) {
    ...
    prevFrameBuf = GetPrevFramePixels();

    // Round the x positions
    x1 = x1 - (x1 % 4);
    x2 = x2 - (x2 % 4) + 4;
    // Can only load 6 rows of the color buffer at a time: 320*6*2 = 3840 bytes of the 4096 TMEM capacity
    stripY = (y2 - y1) / 6;
    extraY = (y2 - y1) % 6;
    ...
    for (i = 0; i < stripY; i++) {
        gDPLoadTextureTile(gMainGfxPos++, osVirtualToPhysical(prevFrameBuf), G_IM_FMT_RGBA, G_IM_SIZ_16b, 320, 6,
    ...
    if (extraY != 0) {
        gDPLoadTextureTile(gMainGfxPos++, osVirtualToPhysical(prevFrameBuf), G_IM_FMT_RGBA, G_IM_SIZ_16b, 320, extraY,
...
void draw_prev_frame_buffer_at_screen_pos(s32 x1, s32 y1, s32 x2, s32 y2, f32 alpha) {
    ...
    // Request framebuffer capture so next frame's data is available.
    // The capture countdown (3 frames) ensures we keep capturing while the effect is active.
    RequestPrevFrameCapture();
