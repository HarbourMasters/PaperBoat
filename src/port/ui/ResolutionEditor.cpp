...
void ResolutionCustomWidget(WidgetInfo& info) {
    ...
    UIWidgets::CVarCheckbox(
        ...
        UIWidgets::CheckboxOptions({ { .disabled = disabled_everything } })
            .Tooltip(
                "Override the resolution scale slider and use the settings "
    ...
    if (ImGui::Combo(
            "Pixel Count Presets", &item_pixelCount, pixelCountPresetLabels,
    ...
    if (showHorizontalResField) {
        ...
        if ((aspectRatioX > 0.0f) && (aspectRatioY > 0.0f)) {
            ...
            ImGui::TextColored(
                messageColor[MESSAGE_QUESTION], ICON_FA_QUESTION_CIRCLE " \"Force aspect ratio\" required."
            );
            // ImGui::Text(" ");
            ImGui::SameLine();
    ...
    if (ImGui::CollapsingHeader("Integer Scaling Settings", IntegerScalingResolvedImGuiFlag)) {
        ...
        UIWidgets::CVarSliderInt(
            ...
            UIWidgets::IntSliderOptions(
                ...
                .DefaultValue(1)
                .Tooltip(
                    "Integer scales the image. Only available in Pixel "
                    "Perfect Mode."
                )
                .Color(THEME_COLOR)
        );
        ...
        UIWidgets::CVarCheckbox(
            ...
            UIWidgets::CheckboxOptions({ { .disabled = disabled_pixelPerfectMode } })
                ...
                .Color(THEME_COLOR)
                .Tooltip(
                    "Automatically sets scale factor to fit window. Only "
                    "available in Pixel "
                    "Perfect Mode."
                )
        );
        ...
        // Beginning of Integer Scaling additional settings.
        {
            ...
            if (UIWidgets::CVarCheckbox(
                    ...
                    UIWidgets::CheckboxOptions({ { .disabled = disabled_neverExceedBounds } })
                        .Tooltip(
                            "Prevents integer scaling factor from exceeding "
                            "screen bounds.\n\n"
...
void RegisterResolutionWidgets() {
    auto fastWnd = dynamic_pointer_cast<Fast::Fast3dWindow>(WindowGetWindowComponent());
    ...
    mPaperboatMenu->AddWidget(path, "Internal resolution: {} x {}", WIDGET_TEXT)
        ...
        .PreFunc([](WidgetInfo& info) {
            ...
            info.name = fmt::format(
                "Internal resolution: {} x {}", gfx_current_dimensions.width, gfx_current_dimensions.height
            );
        });
    ...
    mPaperboatMenu
        ->AddWidget(
            path, ICON_FA_EXCLAMATION_TRIANGLE " Significant frame rate (FPS) drops may be occuring.", WIDGET_TEXT
        )
        .RaceDisable(false)
        .PreFunc([](WidgetInfo& info) {
    ...
    mPaperboatMenu->AddWidget(path, ICON_FA_QUESTION_CIRCLE " \"N64 Mode\" is overriding these settings.", WIDGET_TEXT)
        .RaceDisable(false)
    ...
    // Aspect Ratio
    mPaperboatMenu->AddWidget(path, "AspectSep", WIDGET_SEPARATOR).RaceDisable(false).PreFunc([](WidgetInfo& info) {
        if (mPaperboatMenu->GetDisabledMap().at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF).active) {
    ...
    mPaperboatMenu->AddWidget(path, "Aspect Ratio", WIDGET_COMBOBOX)
        ...
        .Callback([](WidgetInfo& info) {
            if (item_aspectRatio != default_aspectRatio) { // don't change anything if "Custom" is
                                                           // selected.
