...
/*  Console Variables are grouped under gAdvancedResolution. (e.g.
   CVAR_PREFIX_ADVANCED_RESOLUTION
   ".Enabled")

    The following cvars are used in Libultraship and can be edited here:
        - Enabled                                       - Turns Advanced
   Resolution Mode on.
        - AspectRatioX, AspectRatioY                    - Aspect ratio controls.
   To toggle off, set either to zero.
        - VerticalPixelCount, VerticalResolutionToggle  - Resolution controls.
        - PixelPerfectMode, IntegerScale.Factor         - Pixel Perfect Mode
   a.k.a. integer scaling controls.
        - IntegerScale.FitAutomatically                 - Automatic resizing for
   Pixel Perfect Mode.
        - IntegerScale.NeverExceedBounds                - Prevents manual
   resizing from exceeding screen bounds.

    The following cvars are also implemented in LUS for niche use cases:
        - IgnoreAspectCorrection                        - Stretch framebuffer to
   fill screen. This is something of a power-user setting for niche setups that
   most people won't need or care about, but may be useful if playing the
   Switch/Wii U ports on a 4:3 television.
        - IntegerScale.ExceedBoundsBy                   - Offset the max screen
   bounds, usually by +1. This isn't that useful at the moment, so it's unused
   here.
*/
...
std::unordered_map<int32_t, const char *> aspectRatioPresetLabels = {
    ...
    {5, "16:10 (8:5)"},
    {6, "Ultrawide (21:9)"}};
const float aspectRatioPresetsX[] = {0.0f, 16.0f, 4.0f, 16.0f,
                                     5.0f, 16.0f, 21.0f};
...
const char *pixelCountPresetLabels[] = {
    "Custom",     "Native N64 (240p)", "2x (480p)",
    "3x (720p)",  "4x (960p)",         "5x (1200p)",
    "6x (1440p)", "Full HD (1080p)",   "4K (2160p)"};
const int pixelCountPresets[] = {480,  240,  480,  720, 960,
                                 1200, 1440, 1080, 2160};
...
const unsigned short default_maxIntegerScaleFactor =
    6; // Default size of Integer scale factor slider.
...
static short max_integerScaleFactor =
    default_maxIntegerScaleFactor; // default value, which may or may not get
                                   // overridden depending on viewport res

static short integerScale_maximumBounds =
    1; // can change when window is resized
...
void ResolutionCustomWidget(WidgetInfo &info) {
  ...
  UIWidgets::CVarCheckbox(
      ...
      UIWidgets::CheckboxOptions({{.disabled = disabled_everything}})
          ...
          .Color(THEME_COLOR));
  // if (disabled_pixelCount || disabled_everything) { // Hide pixel count
  // controls.
  //     UIWidgets::DisableComponent(ImGui::GetStyle().Alpha * 0.5f);
  // }
  UIWidgets::PushStyleCombobox(THEME_COLOR);
  if (ImGui::Combo("Pixel Count Presets", &item_pixelCount,
                   pixelCountPresetLabels,
                   IM_ARRAYSIZE(pixelCountPresetLabels)) &&
      item_pixelCount != default_pixelCount) { // don't change anything if
                                               // "Custom" is selected.
    ...
    CVarSetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION ".VerticalPixelCount",
                   verticalPixelCount);
  ...
  if (showHorizontalResField) {
    ...
    if ((aspectRatioX > 0.0f) && (aspectRatioY > 0.0f)) {
      ...
      if (ImGui::InputInt("Horiz. Pixel Count", &horizontalPixelCount, 8,
                          320)) {
      ...
      if (UIWidgets::Button("Click to resolve",
        ...
        horizontalPixelCount =
            (verticalPixelCount / aspectRatioY) * aspectRatioX;
      }
    }
  }
  ...
  if (ImGui::CollapsingHeader("Integer Scaling Settings",
                              IntegerScalingResolvedImGuiFlag)) {
    ...
    UIWidgets::CVarSliderInt(
        ...
        UIWidgets::IntSliderOptions(
            {{.disabled = disabled_pixelPerfectMode ||
                          CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
                                         ".IntegerScale.FitAutomatically",
                                         0)}})
            .Min(1)
            .Max(max_integerScaleFactor)
            .DefaultValue(1)
            .Tooltip("Integer scales the image. Only available in Pixel "
                     "Perfect Mode.")
            .Color(THEME_COLOR));
    // Display warning if size is being clamped or if framebuffer is larger than
    // viewport.
    if (!disabled_pixelPerfectMode &&
        (CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
                        ".IntegerScale.NeverExceedBounds",
                        1) &&
         CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION ".IntegerScale.Factor",
                        1) > integerScale_maximumBounds)) {
      ImGui::SameLine();
      ImGui::TextColored(messageColor[MESSAGE_WARNING],
                         ICON_FA_EXCLAMATION_TRIANGLE " Window exceeded.");
    }
    ...
    UIWidgets::CVarCheckbox(
        ...
        UIWidgets::CheckboxOptions({{.disabled = disabled_pixelPerfectMode}})
            .DefaultValue(true)
            .Color(THEME_COLOR)
            .Tooltip("Automatically sets scale factor to fit window. Only "
                     "available in Pixel "
                     "Perfect Mode."));
    if (CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
                       ".IntegerScale.FitAutomatically",
                       0)) {
      // This is just here to update the value shown on the slider.
      // The function in LUS to handle this setting will ignore
  ...
  if (ImGui::CollapsingHeader("Additional Settings")) {
    ...
    // Disable aspect correction, stretching the framebuffer to fill the
    // viewport. This option is only really needed on systems limited to 16:9 TV
    // resolutions, such as consoles. The associated cvar is still functional on
    // PC platforms if you want to use it though.
    UIWidgets::CVarCheckbox(
        ...
        UIWidgets::CheckboxOptions(
            {{.disabled = CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
                                         ".PixelPerfectMode",
                                         0) ||
                          disabled_everything}})
            .Color(THEME_COLOR));
    ...
    if (CVarGetInteger(
      ...
      ImGui::TextColored(
          messageColor[MESSAGE_QUESTION], ICON_FA_QUESTION_CIRCLE
    ...
    // A requested addition; an alternative way of displaying the resolution
    // field.
    if (UIWidgets::Checkbox(
      ...
      if (!showHorizontalResField &&
          (aspectRatioX > 0.0f)) { // when turning this setting off
    ...
    // Beginning of Integer Scaling additional settings.
    {
      ...
      if (UIWidgets::CVarCheckbox(
              ...
              UIWidgets::CheckboxOptions(
                  {{.disabled = disabled_neverExceedBounds}})
                  .Tooltip("Prevents integer scaling factor from exceeding "
                           "screen bounds.\n\n"
                           "Enabled: Will clamp the scaling factor and display "
                           "a gentle warning "
                           "in the "
                           "resolution editor.\n"
                           "Disabled: Will allow scaling to exceed screen "
                           "bounds, for users who "
                           "want to crop "
                           "overscan.\n\n"
                           " " ICON_FA_INFO_CIRCLE
                           " Please note that exceeding screen bounds "
                           "may show a scroll bar on-screen.")
                  .Color(THEME_COLOR)
                  .DefaultValue(true))) {

        // Initialise the (currently unused) "Exceed Bounds By" cvar if it's
        // been changed.
        if (CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
      ...
      // A popular feature in some retro frontends/upscalers, sometimes called
      // "crop overscan" or "1080p 5x".
      UIWidgets::CVarCheckbox(
      ...
      // It does actually function as expected, but exceeding the bottom of the
      // screen shows a scroll bar. I've ended up commenting this one out
      // because of the scroll bar, and for simplicity.
      ...
      if (!CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
          ...
          CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
                         ...
                         0)) {
        ImGui::TextColored(
            messageColor[MESSAGE_INFO],
            " " ICON_FA_INFO_CIRCLE
            " A scroll bar may become visible if screen bounds are exceeded.");

        // Another support helper button, to disable the unused "Exceed Bounds
        // By" cvar. (Remove this button if uncommenting the checkbox.)
        if (CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION
                           ".IntegerScale.ExceedBoundsBy",
                           0)) {
          if (UIWidgets::Button(
  ...
  if (update[UPDATE_aspectRatioX] || update[UPDATE_aspectRatioY] ||
      update[UPDATE_verticalPixelCount]) {
    if (update[UPDATE_aspectRatioX]) {
      if (aspectRatioX < 0.0f) {
        aspectRatioX = 0.0f;
      }
      CVarSetFloat(CVAR_PREFIX_ADVANCED_RESOLUTION ".AspectRatioX",
                   aspectRatioX);
    }
    if (update[UPDATE_aspectRatioY]) {
      ...
      CVarSetFloat(CVAR_PREFIX_ADVANCED_RESOLUTION ".AspectRatioY",
                   aspectRatioY);
    }
    if (update[UPDATE_verticalPixelCount]) {
      // There's a upper and lower clamp on the Libultraship side too,
      // so clamping it here is entirely visual, so the vertical resolution
      // field reflects it.
    ...
    CVarSetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION ".UIComboItem.AspectRatio",
                   item_aspectRatio);
    CVarSetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION ".UIComboItem.PixelCount",
                   item_pixelCount);
    Ship::Context::GetInstance()
...
void RegisterResolutionWidgets() {
  ...
  mPaperboatMenu->AddWidget(path, "Viewport dimensions: {} x {}", WIDGET_TEXT)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        auto gfx_current_game_window_viewport =
            GetInterpreter().get()->mGameWindowViewport;
        info.name = fmt::format("Viewport dimensions: {} x {}",
                                gfx_current_game_window_viewport.width,
                                gfx_current_game_window_viewport.height);
      });
  mPaperboatMenu->AddWidget(path, "Internal resolution: {} x {}", WIDGET_TEXT)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        auto gfx_current_dimensions = GetInterpreter().get()->mCurDimensions;
        info.name = fmt::format("Internal resolution: {} x {}",
  ...
  //  Activator
  mPaperboatMenu
      ->AddWidget(path, "Enable advanced settings.", WIDGET_CVAR_CHECKBOX)
  ...
  mPaperboatMenu
      ->AddWidget(path,
                  ...
                  " Significant frame rate (FPS) drops may be occuring.",
                  WIDGET_TEXT)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        info.isHidden =
            !(!CVarGetInteger(CVAR_LOW_RES_MODE, 0) && IsDroppingFrames());
      })
      .Options(TextOptions().Color(Colors::Orange));
  mPaperboatMenu
      ->AddWidget(path,
                  ICON_FA_QUESTION_CIRCLE
                  " \"N64 Mode\" is overriding these settings.",
                  WIDGET_TEXT)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
  ...
  mPaperboatMenu->AddWidget(path, "Click to disable N64 mode", WIDGET_BUTTON)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        info.isHidden = !CVarGetInteger(CVAR_LOW_RES_MODE, 0);
      })
  ...
  // Aspect Ratio
  mPaperboatMenu->AddWidget(path, "AspectSep", WIDGET_SEPARATOR)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        if (mPaperboatMenu->GetDisabledMap()
                .at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF)
                .active) {
          info.activeDisables.push_back(DISABLE_FOR_ADVANCED_RESOLUTION_OFF);
        }
      });
  mPaperboatMenu->AddWidget(path, "Force aspect ratio:", WIDGET_TEXT)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        if (mPaperboatMenu->GetDisabledMap()
                .at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF)
  ...
  mPaperboatMenu->AddWidget(path, "(Select \"Off\" to disable.)", WIDGET_TEXT)
      .RaceDisable(false)
      .PreFunc([](WidgetInfo &info) {
        if (mPaperboatMenu->GetDisabledMap()
                .at(DISABLE_FOR_ADVANCED_RESOLUTION_OFF)
  ...
  // Presets
  mPaperboatMenu->AddWidget(path, "Aspect Ratio", WIDGET_COMBOBOX)
      ...
      .Callback([](WidgetInfo &info) {
        if (item_aspectRatio !=
            default_aspectRatio) { // don't change anything if "Custom" is
                                   // selected.
  ...
  mPaperboatMenu->AddWidget(path, "AspectRatioCustom", WIDGET_CUSTOM)
      .RaceDisable(false)
      .CustomFunction([](WidgetInfo &info) {
        // Hide aspect ratio input fields if using one of the presets.
        if (item_aspectRatio == default_aspectRatio &&
            !showHorizontalResField) {
          // Declare input interaction bools outside of IF statement to prevent
          // Y field from disappearing.
          const bool input_X = UIWidgets::SliderFloat(
              "X", &aspectRatioX,
          ...
          const bool input_Y = UIWidgets::SliderFloat(
              "Y", &aspectRatioY,
              UIWidgets::FloatSliderOptions({{.disabled = disabled_everything}})
                  .Min(0.1f)
                  .Max(24.0f)
          ...
          if (item_aspectRatio) {
            auto gfx_current_dimensions =
                GetInterpreter().get()->mCurDimensions;
            ImGui::Dummy({0, 2});
  ...
  mPaperboatMenu->AddWidget(path, "MoreResolutionSettings", WIDGET_CUSTOM)
...
void UpdateResolutionVars() {
  ...
  // Initialise integer scale bounds.
  short max_integerScaleFactor =
      default_maxIntegerScaleFactor; // default value, which may or may not get
  // overridden depending on viewport res

  short integerScale_maximumBounds = 1; // can change when window is resized
  // This is mostly just for UX purposes, as Fit Automatically logic is part of
  // LUS.
  auto gfx_current_game_window_viewport =
      GetInterpreter().get()->mGameWindowViewport;
  auto gfx_current_dimensions = GetInterpreter().get()->mCurDimensions;
  if (((float)gfx_current_game_window_viewport.width /
       gfx_current_game_window_viewport.height) >
  ...
  // Lower-clamping maximum bounds value to 1 is no-longer necessary as that's
  // accounted for in LUS. Letting it go below 1 in this Editor will even allow
  // for checking if screen bounds are being exceeded.
  ...
  // Combo List defaults
  item_aspectRatio = CVarGetInteger(
      CVAR_PREFIX_ADVANCED_RESOLUTION ".UIComboItem.AspectRatio", 3);
  item_pixelCount =
      CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION ".UIComboItem.PixelCount",
                     default_pixelCount);
  // Stored Values for non-UIWidgets elements
  aspectRatioX = CVarGetFloat(CVAR_PREFIX_ADVANCED_RESOLUTION ".AspectRatioX",
                              aspectRatioPresetsX[item_aspectRatio]);
  aspectRatioY = CVarGetFloat(CVAR_PREFIX_ADVANCED_RESOLUTION ".AspectRatioY",
                              aspectRatioPresetsY[item_aspectRatio]);
  verticalPixelCount =
      CVarGetInteger(CVAR_PREFIX_ADVANCED_RESOLUTION ".VerticalPixelCount",
  ...
  disabled_pixelCount = !CVarGetInteger(
...
static RegisterMenuUpdateFunc updateFunc(UpdateResolutionVars, "Settings",
                                         "Graphics");
