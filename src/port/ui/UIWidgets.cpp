...
// of characters Manually included newlines will still be respected and reset
// the line length If line is midword when it hits the limit, text should break
// at the last encountered space
std::string WrappedText(const char *text, unsigned int charactersPerLine) {
  std::string newText(text);
  const size_t tipLength = newText.length();
  int lastSpace = -1;
  int currentLineLength = 0;
  for (unsigned int currentCharacter = 0; currentCharacter < tipLength;
       currentCharacter++) {
...
void PaddedSeparator(bool padTop, bool padBottom, float extraVerticalTopPadding,
                     float extraVerticalBottomPadding) {
  if (padTop) {
...
void PushStyleMenu(Colors color) { PushStyleMenu(ColorValues.at(color)); }
...
void PushStyleInput(Colors color) { PushStyleInput(ColorValues.at(color)); }
...
void PushStyleHeader(Colors color) { PushStyleHeader(ColorValues.at(color)); }

void PopStyleHeader() { ImGui::PopStyleColor(3); }
...
bool WindowButton(const char *label, const char *cvarName,
  ...
  if (Button(buttonText.c_str(),
             {{options.tooltip, options.disabled, options.disabledTooltip},
              options.size,
              options.padding,
              options.color})) {
...
void Spacer(float height) { ImGui::Dummy(ImVec2(0.0f, height)); }

void Separator(bool padTop, bool padBottom, float extraVerticalTopPadding,
               float extraVerticalBottomPadding) {
  if (padTop) {
    Spacer(extraVerticalTopPadding);
  }
...
bool Checkbox(const char *_label, bool *value, const CheckboxOptions &options) {
  ...
  if (right) {
    float labelOffsetX =
        (above ? 0 : (style.ItemInnerSpacing.x * 2.0f) + square_sz);
    if (!lpFar) {
      pos.x += ImGui::GetContentRegionAvail().x - (label_size.x + labelOffsetX);
    }
  }
  float bbAboveX =
      lpFar ? ImGui::GetContentRegionAvail().x
            : (label_size.x +
               (above ? 0 : (style.ItemInnerSpacing.x * 2.0f) + square_sz));
  float bbAboveY =
      label_size.y + (above ? square_sz : 0) + (style.FramePadding.y * 2.0f);
  ...
  if (options.alignment == ComponentAlignments::Right) {
    ...
    float labelOffsetX =
        above ? 0
              : (lpFar ? labelFarOffset
                       : (style.ItemInnerSpacing.x * 2.0f) + square_sz);
...
bool CVarCheckbox(const char *label, const char *cvarName,
...
bool StateButton(const char *str_id, const char *label, ImVec2 size,
                 ButtonOptions options, ImGuiButtonFlags flags) {
  ...
  ImGui::RenderTextClipped(bb.Min + (style.FramePadding * 0.35f),
                           bb.Max - (style.FramePadding / 4), label, NULL,
                           &label_size, style.ButtonTextAlign, &bb);
  PopStyleButton();
  /*ImGui::RenderArrow(window->DrawList,
  bb.Min +
  ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y -
...
void PushStyleTabs(Colors color) { PushStyleTabs(ColorValues.at(color)); }
...
bool SliderInt(const char *label, int32_t *value,
  ...
  float width = (options.size == ImVec2(0, 0))
                    ? ImGui::GetContentRegionAvail().x
                    : options.size.x;
  if (options.labelPosition == LabelPositions::Near ||
      options.labelPosition == LabelPositions::Far) {
    width = width -
            (ImGui::CalcTextSize(label).x + ImGui::GetStyle().FramePadding.x);
  }
  ...
  if (options.showButtons) {
    if (Button("-",
               ButtonOptions{.color = options.color}.Size(Sizes::Inline)) &&
        *value > options.min) {
      *value -= options.step;
      if (options.clamp) {
    ...
    ImGui::SetNextItemWidth(width - (ImGui::CalcTextSize("+").x +
                                     ImGui::GetStyle().FramePadding.x * 2 + 3) *
                                        2);
  } else {
    ImGui::SetNextItemWidth(width);
  }
  if (ImGui::SliderScalar(invisibleLabel, ImGuiDataType_S32, value,
                          &options.min, &options.max, options.format,
                          options.flags)) {
    if (options.clamp) {
      if (*value < options.min) {
        *value = options.min;
...
bool CVarSliderInt(const char *label, const char *cvarName,
                   const IntSliderOptions &options) {
  bool dirty = false;
...
bool SliderFloat(const char *label, float *value,
  ...
  float maxToDisplay =
      options.isPercentage ? options.max * 100.0f : options.max;
  float minToDisplay =
      options.isPercentage ? options.min * 100.0f : options.min;
  ImGui::PushID(label);
  ImGui::BeginGroup();
  ImGui::BeginDisabled(options.disabled);
  PushStyleSlider(options.color);
  ...
  if (ImGui::SliderScalar(invisibleLabel, ImGuiDataType_Float, &valueToDisplay,
                          &minToDisplay, &maxToDisplay, options.format,
                          options.flags)) {
...
bool CVarSliderFloat(const char *label, const char *cvarName,
                     const FloatSliderOptions &options) {
  bool dirty = false;
...
bool InputString(const char *label, std::string *value,
  ...
  if (ImGui::InputText(label, (char *)value->c_str(), value->capacity() + 1,
                       flags, InputTextResizeCallback, value)) {
    dirty = true;
  }
  if (value->empty() && !options.placeholder.empty()) {
    ImGui::SameLine(17.0f);
    ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 0.4f), "%s",
                       options.placeholder.c_str());
  }
  if (options.hasError) {
    ImGui::PopStyleColor();
  }
...
bool CVarInputString(const char *label, const char *cvarName,
                     const InputOptions &options) {
  bool dirty = false;
...
bool InputInt(const char *label, int32_t *value, const InputOptions &options) {
  ...
  ImGui::SetNextItemWidth(width);
  if (ImGui::InputScalar(label, ImGuiDataType_S32, value, nullptr, nullptr,
                         nullptr, options.addedFlags)) {
    dirty = true;
  }
  if ((ImGui::GetItemStatusFlags() & ImGuiItemStatusFlags_Edited) &&
...
bool CVarInputInt(const char *label, const char *cvarName,
                  const InputOptions &options) {
  bool dirty = false;
...
bool CVarColorPicker(const char *label, const char *cvarName,
                     Color_RGBA8 defaultColor, bool hasAlpha, uint8_t modifiers,
  ...
  ImVec4 colorVec = ImVec4(color.r / 255.0f, color.g / 255.0f, color.b / 255.0f,
                           color.a / 255.0f);
  bool changed = false;
  ...
  if (showReset) {
    ...
    if (UIWidgets::Button(
            ...
            UIWidgets::ButtonOptions(
                {{.tooltip = "Resets this color to its default value"}})
                .Color(themeColor)
                .Size(UIWidgets::Sizes::Inline))) {
      // TODO: Remove for next minor or major version, temporary fix for already
      // migrated configs to 3 for 9.0.0
  ...
  if (showRandom) {
    ...
    if (UIWidgets::Button(
            uniqueTag.c_str(),
            UIWidgets::ButtonOptions(
                {{.tooltip = "Generates a random color value to use"}})
                .Color(themeColor)
                .Size(UIWidgets::Sizes::Inline))) {
  ...
  if (showLock) {
    ...
    UIWidgets::CVarCheckbox(
        ...
        UIWidgets::CheckboxOptions(
            {{.tooltip = "Prevents this color from being changed"}})
            .Color(themeColor));
  }
  if (changed) {
...
bool RadioButton(const char *label, bool active,
                 const RadioButtonsOptions &options) {
  ImGuiWindow *window = ImGui::GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
bool CVarRadioButton(const char *text, const char *cvarName, int32_t id,
                     const RadioButtonsOptions &options) {
...
ImVec4 VecFromRGBA8(Color_RGBA8 color) {
  ImVec4 vec = {color.r / 255.0f, color.g / 255.0f, color.b / 255.0f,
                color.a / 255.0f};
  return vec;
}
