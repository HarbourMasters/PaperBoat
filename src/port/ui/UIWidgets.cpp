...
bool StateButton(const char* str_id, const char* label, ImVec2 size, ButtonOptions options, ImGuiButtonFlags flags) {
    ...
    ImGui::RenderTextClipped(
        bb.Min + (style.FramePadding * 0.35f), bb.Max - (style.FramePadding / 4), label, NULL, &label_size,
...
bool SliderInt(const char* label, int32_t* value, const IntSliderOptions& options) {
    ...
    if (ImGui::SliderScalar(
            invisibleLabel, ImGuiDataType_S32, value, &options.min, &options.max, options.format, options.flags
        ))
    {
        if (options.clamp) {
            if (*value < options.min) {
...
bool SliderFloat(const char* label, float* value, const FloatSliderOptions& options) {
    ...
    if (ImGui::SliderScalar(
            invisibleLabel, ImGuiDataType_Float, &valueToDisplay, &minToDisplay, &maxToDisplay, options.format,
            options.flags
        ))
    {
...
bool CVarColorPicker(
    ...
    if (hasAlpha) {
        changed = ImGui::ColorEdit4(
            label, (float*) &colorVec, flags | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreview
        );
    } else {
...
bool RadioButton(const char* label, bool active, const RadioButtonsOptions& options) {
    ...
    if (style.FrameBorderSize > 0.0f) {
        window->DrawList->AddCircle(
            center + ImVec2(1, 1), radius, ImGui::GetColorU32(ImGuiCol_BorderShadow), num_segment, style.FrameBorderSize
        );
        window->DrawList->AddCircle(
            center, radius, ImGui::GetColorU32(ImGuiCol_Border), num_segment, style.FrameBorderSize
        );
    }
