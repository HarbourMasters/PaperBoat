...
void PaperboatMenu::AddMenuSettings() {
  ...
  AddWidget(path, "Internal Resolution", WIDGET_CVAR_SLIDER_FLOAT)
      ...
      .PreFunc([](WidgetInfo &info) {
        if (mPaperboatMenu->disabledMap.at(DISABLE_FOR_ADVANCED_RESOLUTION_ON)
                .active &&
            mPaperboatMenu->disabledMap.at(DISABLE_FOR_VERTICAL_RES_TOGGLE_ON)
                .active) {
        ...
        } else if (mPaperboatMenu->disabledMap.at(DISABLE_FOR_LOW_RES_MODE_ON)
                       .active) {
          info.activeDisables.push_back(DISABLE_FOR_LOW_RES_MODE_ON);
        }
      })
      .Options(FloatSliderOptions()
                   .Tooltip("Multiplies your output resolution by the value "
  ...
  AddWidget(path, "Anti-aliasing (MSAA)", WIDGET_CVAR_SLIDER_INT)
      ...
      .Options(IntSliderOptions()
                   .Tooltip("Activates MSAA (multi-sample anti-aliasing) from "
  ...
  AddWidget(path, "Current FPS", WIDGET_CVAR_SLIDER_INT)
      ...
      .PreFunc([](WidgetInfo &info) {
        if (mPaperboatMenu->disabledMap.at(DISABLE_FOR_MATCH_REFRESH_RATE_ON)
                .active)
  ...
  AddWidget(path, "Enable Vsync", WIDGET_CVAR_CHECKBOX)
      ...
      .PreFunc([](WidgetInfo &info) {
        info.isHidden =
            mPaperboatMenu->disabledMap.at(DISABLE_FOR_NO_VSYNC).active;
      })
      .Options(CheckboxOptions()
                   .Tooltip("Removes tearing, but clamps your max FPS to your "
  ...
  AddWidget(path, "Windowed Fullscreen", WIDGET_CVAR_CHECKBOX)
      ...
      .PreFunc([](WidgetInfo &info) {
        info.isHidden =
            mPaperboatMenu->disabledMap.at(DISABLE_FOR_NO_WINDOWED_FULLSCREEN)
                .active;
      })
  ...
  AddWidget(path, "Allow multi-windows", WIDGET_CVAR_CHECKBOX)
      ...
      .PreFunc([](WidgetInfo &info) {
        info.isHidden =
            mPaperboatMenu->disabledMap.at(DISABLE_FOR_NO_MULTI_VIEWPORT)
                .active;
      })
      .Options(CheckboxOptions()
                   .Tooltip("Allows multiple windows to be opened at once. "
  ...
  AddWidget(path, "Popout Input Viewer Settings", WIDGET_WINDOW_BUTTON)
      ...
      .HideInSearch(true)
      .Options(WindowButtonOptions().Tooltip(
          "Enables the separate Input Viewer Settings Window."));
  ...
  AddWidget(path, "Test Achievement Notification", WIDGET_BUTTON)
      .RaceDisable(false)
      .Callback([](WidgetInfo &info) {
        Notification::EmitAchievement(nullptr, "Harbour Mastery", 0);
      })
      .Options(ButtonOptions().Tooltip("Displays a test notification."));
}
