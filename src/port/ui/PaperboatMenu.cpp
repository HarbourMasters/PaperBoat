...
void PaperboatMenu::AddSidebarEntry(std::string sectionName,
                                    std::string sidebarName,
                                    uint32_t columnCount) {
  assert(!sectionName.empty());
  assert(!sidebarName.empty());
  menuEntries.at(sectionName)
      .sidebars.emplace(sidebarName, SidebarEntry{.columnCount = columnCount});
  menuEntries.at(sectionName).sidebarOrder.push_back(sidebarName);
}

WidgetInfo &PaperboatMenu::AddWidget(WidgetPath &pathInfo,
                                     std::string widgetName,
                                     WidgetType widgetType) {
  assert(!widgetName.empty()); // Must be unique
  assert(menuEntries.contains(
      pathInfo.sectionName)); // Section/header must already exist
  assert(menuEntries.at(pathInfo.sectionName)
             .sidebars.contains(
                 pathInfo.sidebarName)); // Sidebar must already exist
  std::unordered_map<std::string, SidebarEntry> &sidebar =
      menuEntries.at(pathInfo.sectionName).sidebars;
  ...
  entry.columnWidgets.at(column).push_back(
      {.name = widgetName, .type = widgetType});
...
PaperboatMenu::PaperboatMenu(const std::string &consoleVariable,
...
void PaperboatMenu::InitElement() {
  ...
  disabledMap = {
      ...
      {DISABLE_FOR_MATCH_REFRESH_RATE_ON,
       {[](disabledInfo &info) -> bool {
          return CVarGetInteger(CVAR_SETTING("MatchRefreshRate"), 0);
        },
...
void PaperboatMenu::UpdateElement() { Ship::Menu::UpdateElement(); }

void PaperboatMenu::Draw() { Ship::Menu::Draw(); }

void PaperboatMenu::DrawElement() { Ship::Menu::DrawElement(); }
} // namespace PaperboatGui
