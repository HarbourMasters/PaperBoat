...
PaperboatInputEditorWindow::~PaperboatInputEditorWindow() {}

void PaperboatInputEditorWindow::OnInit(const nlohmann::json& initArgs) {
  ...
  mButtonsBitmasks = {BTN_A, BTN_B,   BTN_START, BTN_L,     BTN_R,
                      BTN_Z, BTN_CUP, BTN_CDOWN, BTN_CLEFT, BTN_CRIGHT};
...
void PaperboatInputEditorWindow::UpdateElement() {
  ...
  if (mInputEditorPopupOpen &&
      ImGui::IsPopupOpen("", ImGuiPopupFlags_AnyPopupId)) {
    ControllerGetControlDeck()->BlockGameInput(
    ...
    if (mGameInputBlockTimer != INT32_MAX) {
      ...
      if (mGameInputBlockTimer <= 0) {
        ControllerGetControlDeck()->UnblockGameInput(
            INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
        mGameInputBlockTimer = INT32_MAX;
      }
    }

    if (WindowGetWindowComponent()
            ->GetGui()
...
void PaperboatInputEditorWindow::DrawAnalogPreview(const char *label,
                                                   ImVec2 stick, float deadzone,
                                                   bool gyro) {
  ImGui::BeginChild(label,
                    ImVec2(gyro ? SCALE_IMGUI_SIZE(78) : SCALE_IMGUI_SIZE(96),
                           SCALE_IMGUI_SIZE(85)),
                    false);
  ImGui::SetCursorPos(
      ImVec2(ImGui::GetCursorPos().x + gyro ? SCALE_IMGUI_SIZE(10)
                                            : SCALE_IMGUI_SIZE(18),
  ...
  drawList->AddRect(ImVec2(borderSquareLeft, borderSquareTop),
                    ImVec2(borderSquareLeft + borderSquareSize,
                           borderSquareTop + borderSquareSize),
  ...
  ImVec2 joystickCenterpoint =
      ImVec2(cursorScreenPosition.x + cardinalRadius + SCALE_IMGUI_SIZE(12),
             cursorScreenPosition.y + cardinalRadius + SCALE_IMGUI_SIZE(11));
  drawList->AddQuadFilled(
      joystickCenterpoint,
      ImVec2(joystickCenterpoint.x - diagonalRadius,
             joystickCenterpoint.y + diagonalRadius),
  ...
  if ((stick.x * stick.x + stick.y * stick.y) > (deadzone * deadzone)) {
    joystickIndicatorDistanceFromCenter =
        ImVec2((stick.x * (cardinalRadius / 85.0f)),
               -(stick.y * (cardinalRadius / 85.0f)));
  }
  float indicatorRadius = SCALE_IMGUI_SIZE(5.0f);
  drawList->AddCircleFilled(
      ImVec2(joystickCenterpoint.x + joystickIndicatorDistanceFromCenter.x,
             joystickCenterpoint.y + joystickIndicatorDistanceFromCenter.y),
      indicatorRadius, ImColor(34, 51, 76, 255), 7);

  if (!gyro) {
    ImGui::SetCursorPos(ImVec2(ImGui::GetCursorPos().x - SCALE_IMGUI_SIZE(8),
                               ImGui::GetCursorPos().y + SCALE_IMGUI_SIZE(72)));
    ImGui::Text("X:%3d, Y:%3d", static_cast<int32_t>(stick.x),
                static_cast<int32_t>(stick.y));
  }
  ImGui::EndChild();
}
...
void PaperboatInputEditorWindow::GetButtonColorsForDeviceType(
    Ship::PhysicalDeviceType lusIndex, ImVec4 &buttonColor,
...
void PaperboatInputEditorWindow::DrawInputChip(
    const char *buttonName, ImVec4 color = CHIP_COLOR_N64_GREY) {
  ImGui::BeginDisabled();
...
void PaperboatInputEditorWindow::DrawButtonLineAddMappingButton(
    uint8_t port, N64ButtonMask bitmask) {
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  auto popupId =
      StringHelper::Sprintf("addButtonMappingPopup##%d-%d", port, bitmask);
  if (ImGui::Button(StringHelper::Sprintf("%s###addButtonMappingButton%d-%d",
                                          ICON_FA_PLUS, port, bitmask)
                        .c_str(),
  ...
  if (ImGui::BeginPopup(popupId.c_str())) {
    ...
    // todo: figure out why optional params (using id = "" in the definition)
    // wasn't working
    if (mMappingInputBlockTimer == INT32_MAX &&
...
void PaperboatInputEditorWindow::DrawButtonLineEditMappingButton(
    uint8_t port, N64ButtonMask bitmask, std::string id) {
  auto mapping = ControllerGetControlDeck()
                     ->GetControllerByPort(port)
                     ->GetButton(bitmask)
  ...
  auto physicalInputDisplayName = StringHelper::Sprintf(
      "%s %s", icon.c_str(), mapping->GetPhysicalInputName().c_str());
  GetButtonColorsForDeviceType(mapping->GetPhysicalDeviceType(), buttonColor,
                               buttonHoveredColor);
  ImGui::PushStyleColor(ImGuiCol_Button, buttonColor);
  ImGui::PushStyleColor(ImGuiCol_ButtonHovered, buttonHoveredColor);
  auto popupId =
      StringHelper::Sprintf("editButtonMappingPopup##%s", id.c_str());
  if (ImGui::Button(
          StringHelper::Sprintf("%s###editButtonMappingButton%s",
                                physicalInputDisplayName.c_str(), id.c_str())
              .c_str(),
          ImVec2(ImGui::CalcTextSize(physicalInputDisplayName.c_str()).x +
                     SCALE_IMGUI_SIZE(12.0f),
                 0.0f))) {
    ImGui::OpenPopup(popupId.c_str());
    OffsetMappingPopup();
  }
  ...
  auto sdlAxisDirectionToButtonMapping =
      std::dynamic_pointer_cast<Ship::SDLAxisDirectionToButtonMapping>(mapping);
  if (sdlAxisDirectionToButtonMapping != nullptr) {
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
    auto popupId =
        StringHelper::Sprintf("editAxisThresholdPopup##%s", id.c_str());
    if (ImGui::Button(
            StringHelper::Sprintf("%s###editAxisThresholdButton%s", ICON_FA_COG,
                                  id.c_str())
                .c_str(),
            ImVec2(ImGui::CalcTextSize(ICON_FA_COG).x + SCALE_IMGUI_SIZE(10.0f),
                   0.0f))) {
      ImGui::OpenPopup(popupId.c_str());
      OffsetMappingPopup();
    }
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
      ...
      ImGui::Text("Axis Threshold\n\nThe extent to which the joystick\nmust be "
                  "moved or the trigger\npressed to "
                  "initiate the assigned\nbutton action.\n\n");

      auto globalSettings = ControllerGetControlDeck()
      ...
      if (sdlAxisDirectionToButtonMapping->AxisIsStick()) {
        ...
        int32_t stickAxisThreshold =
            globalSettings->GetStickAxisThresholdPercentage();
        if (stickAxisThreshold == 0) {
          ImGui::BeginDisabled();
        }
        ImGui::PushButtonRepeat(true);
        if (ImGui::Button(
                StringHelper::Sprintf("-##Stick Axis Threshold%s", id.c_str())
                    .c_str())) {
      ...
      if (sdlAxisDirectionToButtonMapping->AxisIsTrigger()) {
        ...
        int32_t triggerAxisThreshold =
            globalSettings->GetTriggerAxisThresholdPercentage();
        if (triggerAxisThreshold == 0) {
          ImGui::BeginDisabled();
        }
        ImGui::PushButtonRepeat(true);
        if (ImGui::Button(
                StringHelper::Sprintf("-##Trigger Axis Threshold%s", id.c_str())
                    .c_str())) {
          globalSettings->SetTriggerAxisThresholdPercentage(
              triggerAxisThreshold - 1);
          globalSettings->SaveToConfig();
        }
        ImGui::PopButtonRepeat();
        if (triggerAxisThreshold == 0) {
  ...
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  if (ImGui::Button(
          StringHelper::Sprintf("%s###removeButtonMappingButton%s",
                                ICON_FA_TIMES, id.c_str())
              .c_str(),
          ImVec2(ImGui::CalcTextSize(ICON_FA_TIMES).x + SCALE_IMGUI_SIZE(10.0f),
                 0.0f))) {
    ControllerGetControlDeck()
        ->GetControllerByPort(port)
        ->GetButton(bitmask)
...
void PaperboatInputEditorWindow::DrawButtonLine(
    const char *buttonName, uint8_t port, N64ButtonMask bitmask,
...
void PaperboatInputEditorWindow::DrawStickDirectionLineAddMappingButton(
    uint8_t port, uint8_t stick, Ship::Direction direction) {
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  auto popupId = StringHelper::Sprintf(
      "addStickDirectionMappingPopup##%d-%d-%d", port, stick, direction);
  if (ImGui::Button(
          StringHelper::Sprintf("%s###addStickDirectionMappingButton%d-%d-%d",
                                ICON_FA_PLUS, port, stick, direction)
              .c_str(),
          ImVec2(SCALE_IMGUI_SIZE(20.0f), 0.0f))) {
    ImGui::OpenPopup(popupId.c_str());
    OffsetMappingPopup();
...
void PaperboatInputEditorWindow::DrawStickDirectionLineEditMappingButton(
    uint8_t port, uint8_t stick, Ship::Direction direction, std::string id) {
  std::shared_ptr<Ship::ControllerAxisDirectionMapping> mapping = nullptr;
  if (stick == Ship::LEFT) {
    mapping = ControllerGetControlDeck()
                  ->GetControllerByPort(port)
                  ->GetLeftStick()
  ...
  ImGui::PushStyleColor(ImGuiCol_ButtonHovered, buttonHoveredColor);
  auto popupId =
      StringHelper::Sprintf("editStickDirectionMappingPopup##%s", id.c_str());
  if (ImGui::Button(
          StringHelper::Sprintf("%s###editStickDirectionMappingButton%s",
                                physicalInputDisplayName.c_str(), id.c_str())
              .c_str(),
          ImVec2(ImGui::CalcTextSize(physicalInputDisplayName.c_str()).x +
                     SCALE_IMGUI_SIZE(12.0f),
  ...
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  if (ImGui::Button(
          StringHelper::Sprintf("%s###removeStickDirectionMappingButton%s",
                                ICON_FA_TIMES, id.c_str())
              .c_str(),
          ImVec2(ImGui::CalcTextSize(ICON_FA_TIMES).x + SCALE_IMGUI_SIZE(10.0f),
                 0.0f))) {
...
void PaperboatInputEditorWindow::DrawStickDirectionLine(
    const char *axisDirectionName, uint8_t port, uint8_t stick,
...
void PaperboatInputEditorWindow::DrawStickSection(
    uint8_t port, uint8_t stick, ImVec4 color = CHIP_COLOR_N64_GREY) {
  static int8_t sX, sY;
  std::shared_ptr<Ship::ControllerStick> controllerStick = nullptr;
  if (stick == Ship::LEFT) {
    controllerStick = ControllerGetControlDeck()
                          ->GetControllerByPort(port)
                          ->GetLeftStick();
  } else {
    controllerStick = ControllerGetControlDeck()
  ...
  DrawAnalogPreview(StringHelper::Sprintf("##AnalogPreview%d", stick).c_str(),
                    ImVec2(sX, sY));

  ImGui::SameLine();
  ImGui::BeginGroup();
  DrawStickDirectionLine(
      StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_UP, stick).c_str(), port,
      stick, Ship::UP, color);
  DrawStickDirectionLine(
      StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_DOWN, stick).c_str(), port,
      stick, Ship::DOWN, color);
  DrawStickDirectionLine(
      StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_LEFT, stick).c_str(), port,
      stick, Ship::LEFT, color);
  DrawStickDirectionLine(
      StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_RIGHT, stick).c_str(), port,
      stick, Ship::RIGHT, color);
  ImGui::EndGroup();
  ImGui::SetNextItemOpen(true, ImGuiCond_Once);
  if (ImGui::TreeNode(
          StringHelper::Sprintf("Analog Stick Options##%d", stick).c_str())) {
    ...
    ImGui::PushButtonRepeat(true);
    if (ImGui::Button(
            StringHelper::Sprintf("-##Sensitivity%d", stick).c_str())) {
    ...
    if (!controllerStick->SensitivityIsDefault()) {
      ImGui::SameLine();
      if (ImGui::Button(StringHelper::Sprintf(
                            "Reset to Default###resetStickSensitivity%d", stick)
                            .c_str())) {
    ...
    ImGui::SameLine(0.0f, 0.0f);
    ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
    if (ImGui::SliderInt(StringHelper::Sprintf("##Deadzone%d", stick).c_str(),
                         &deadzonePercentage, 0, 100, "%d%%",
                         ImGuiSliderFlags_AlwaysClamp)) {
    ...
    if (!controllerStick->DeadzoneIsDefault()) {
      ImGui::SameLine();
      if (ImGui::Button(StringHelper::Sprintf(
                            "Reset to Default###resetStickDeadzone%d", stick)
                            .c_str())) {
    ...
    ImGui::PushButtonRepeat(true);
    if (ImGui::Button(
            StringHelper::Sprintf("-##NotchProximityThreshold%d", stick)
                .c_str())) {
    ...
    ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
    if (ImGui::SliderInt(
            StringHelper::Sprintf("##NotchProximityThreshold%d", stick).c_str(),
            &notchSnapAngle, 0, 45, "%d°", ImGuiSliderFlags_AlwaysClamp)) {
    ...
    if (!controllerStick->NotchSnapAngleIsDefault()) {
      ImGui::SameLine();
      if (ImGui::Button(StringHelper::Sprintf(
                            "Reset to Default###resetStickSnap%d", stick)
                            .c_str())) {
...
void PaperboatInputEditorWindow::UpdateBitmaskToMappingIds(uint8_t port) {
  // todo: do we need this now that ControllerButton exists?

  for (auto [bitmask, button] : ControllerGetControlDeck()
                                    ->GetControllerByPort(port)
                                    ->GetAllButtons()) {
    for (auto [id, mapping] : button->GetAllButtonMappings()) {
      // using a vector here instead of a set because i want newly added
      // mappings to go to the end of the list instead of autosorting
...
void PaperboatInputEditorWindow::UpdateStickDirectionToMappingIds(
    uint8_t port) {
  // todo: do we need this?
  for (auto stick :
       {std::make_pair<uint8_t, std::shared_ptr<Ship::ControllerStick>>(
            Ship::LEFT, ControllerGetControlDeck()
                            ->GetControllerByPort(port)
                            ->GetLeftStick()),
        std::make_pair<uint8_t, std::shared_ptr<Ship::ControllerStick>>(
            Ship::RIGHT, ControllerGetControlDeck()
                             ->GetControllerByPort(port)
                             ->GetRightStick())}) {
    for (auto direction : {Ship::LEFT, Ship::RIGHT, Ship::UP, Ship::DOWN}) {
      for (auto [id, mapping] :
           stick.second->GetAllAxisDirectionMappingByDirection(direction)) {
        // using a vector here instead of a set because i want newly added
        // mappings to go to the end of the list instead of autosorting
        if (std::find(
                mStickDirectionToMappingIds[port][stick.first][direction]
                    .begin(),
                mStickDirectionToMappingIds[port][stick.first][direction].end(),
                id) ==
            mStickDirectionToMappingIds[port][stick.first][direction].end()) {
          mStickDirectionToMappingIds[port][stick.first][direction].push_back(
              id);
        }
      }
    }
...
void PaperboatInputEditorWindow::DrawRemoveRumbleMappingButton(uint8_t port,
                                                               std::string id) {
  ImGui::SameLine();
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  if (ImGui::Button(StringHelper::Sprintf("%s###removeRumbleMapping%s",
                                          ICON_FA_TIMES, id.c_str())
                        .c_str(),
...
void PaperboatInputEditorWindow::DrawAddRumbleMappingButton(uint8_t port) {
  ImGui::SameLine();
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  auto popupId = StringHelper::Sprintf("addRumbleMappingPopup##%d", port);
  if (ImGui::Button(
          StringHelper::Sprintf("%s###addRumbleMapping%d", ICON_FA_PLUS, port)
              .c_str(),
          ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
    ImGui::OpenPopup(popupId.c_str());
    OffsetMappingPopup();
...
bool PaperboatInputEditorWindow::TestingRumble() {
  return mRumbleTimer != INT32_MAX;
}

void PaperboatInputEditorWindow::DrawRumbleSection(uint8_t port) {
  for (auto [id, mapping] : ControllerGetControlDeck()
                                ->GetControllerByPort(port)
                                ->GetRumble()
    ...
    GetButtonColorsForDeviceType(mapping->GetPhysicalDeviceType(), buttonColor,
                                 buttonHoveredColor);
    // begin hackaround
    // https://github.com/ocornut/imgui/issues/282#issuecomment-123763192 spaces
    // to have background color for text in a tree node
    std::string spaces = "";
    for (size_t i = 0; i < mapping->GetPhysicalDeviceName().length(); i++) {
      spaces += " ";
    }
    auto open = ImGui::TreeNode(
        StringHelper::Sprintf("%s###Rumble%s", spaces.c_str(), id.c_str())
            .c_str());
    ImGui::SameLine();
    ...
    ImGui::SameLine();
    if (ImGui::Button(StringHelper::Sprintf("%s###rumbleTestButton%s",
                                            TestingRumble() ? "Stop" : "Test",
                                            id.c_str())
                          .c_str())) {
      if (mRumbleTimer != INT32_MAX) {
        mRumbleTimer = INT32_MAX;
    ...
    if (open) {
      ...
      int32_t smallMotorIntensity =
          mapping->GetHighFrequencyIntensityPercentage();
      if (smallMotorIntensity == 0) {
        ImGui::BeginDisabled();
      }
      ImGui::PushButtonRepeat(true);
      if (ImGui::Button(
              StringHelper::Sprintf("-##Small Motor Intensity%s", id.c_str())
                  .c_str())) {
      ...
      if (!mapping->HighFrequencyIntensityIsDefault()) {
        ...
        if (ImGui::Button(
                StringHelper::Sprintf(
                    "Reset to Default###resetHighFrequencyIntensity%s",
                    id.c_str())
                    .c_str())) {
      ...
      int32_t largeMotorIntensity =
          mapping->GetLowFrequencyIntensityPercentage();
      if (largeMotorIntensity == 0) {
        ImGui::BeginDisabled();
      }
      ImGui::PushButtonRepeat(true);
      if (ImGui::Button(
              StringHelper::Sprintf("-##Large Motor Intensity%s", id.c_str())
                  .c_str())) {
      ...
      if (!mapping->LowFrequencyIntensityIsDefault()) {
        ImGui::SameLine();
        if (ImGui::Button(StringHelper::Sprintf(
                              "Reset to Default###resetLowFrequencyIntensity%s",
                              id.c_str())
                              .c_str())) {
...
void PaperboatInputEditorWindow::DrawRemoveLEDMappingButton(uint8_t port,
                                                            std::string id) {
  ImGui::SameLine();
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  if (ImGui::Button(StringHelper::Sprintf("%s###removeLEDMapping%s",
                                          ICON_FA_TIMES, id.c_str())
                        .c_str(),
                    ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
    ControllerGetControlDeck()
...
void PaperboatInputEditorWindow::DrawAddLEDMappingButton(uint8_t port) {
  ImGui::SameLine();
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  auto popupId = StringHelper::Sprintf("addLEDMappingPopup##%d", port);
  if (ImGui::Button(
          StringHelper::Sprintf("%s###addLEDMapping%d", ICON_FA_PLUS, port)
              .c_str(),
          ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
    ImGui::OpenPopup(popupId.c_str());
    OffsetMappingPopup();
...
void PaperboatInputEditorWindow::DrawLEDSection(uint8_t port) {
  for (auto [id, mapping] : ControllerGetControlDeck()
                                ->GetControllerByPort(port)
                                ->GetLED()
                                ->GetAllLEDMappings()) {
    ...
    auto open = ImGui::TreeNode(
        StringHelper::Sprintf(
            "%s##LED%s", mapping->GetPhysicalDeviceName().c_str(), id.c_str())
            .c_str());
    ...
    if (open) {
      ...
      if (ImGui::Combo(StringHelper::Sprintf("###ledColorSource%s",
                                             mapping->GetLEDMappingId().c_str())
                           .c_str(),
      ...
      if (mapping->GetColorSource() == LED_COLOR_SOURCE_SET) {
        ImGui::SameLine();
        ImVec4 color = {mapping->GetSavedColor().r / 255.0f,
                        mapping->GetSavedColor().g / 255.0f,
                        mapping->GetSavedColor().b / 255.0f, 1.0f};
        if (ImGui::ColorEdit3(
                StringHelper::Sprintf("###ledSavedColor%s",
                                      mapping->GetLEDMappingId().c_str())
                    .c_str(),
                (float *)&color,
                ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel)) {
          mapping->SetSavedColor(
              Color_RGB8({static_cast<uint8_t>(color.x * 255.0),
                          static_cast<uint8_t>(color.y * 255.0),
...
void PaperboatInputEditorWindow::DrawRemoveGyroMappingButton(uint8_t port,
                                                             std::string id) {
  ImGui::SameLine();
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  if (ImGui::Button(StringHelper::Sprintf("%s###removeGyroMapping%s",
                                          ICON_FA_TIMES, id.c_str())
                        .c_str(),
                    ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
    ControllerGetControlDeck()
...
void PaperboatInputEditorWindow::DrawAddGyroMappingButton(uint8_t port) {
  ImGui::SameLine();
  ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
  auto popupId = StringHelper::Sprintf("addGyroMappingPopup##%d", port);
  if (ImGui::Button(
          StringHelper::Sprintf("%s###addGyroMapping%d", ICON_FA_PLUS, port)
              .c_str(),
          ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f)))) {
    ImGui::OpenPopup(popupId.c_str());
    OffsetMappingPopup();
...
void PaperboatInputEditorWindow::DrawGyroSection(uint8_t port) {
  auto mapping = ControllerGetControlDeck()
                     ->GetControllerByPort(port)
                     ->GetGyro()
                     ->GetGyroMapping();
  if (mapping != nullptr) {
    ...
    // the preview window expects values in an n64 analog stick range (-85 to
    // 85) so I decided to multiply these by 85/21
    DrawAnalogPreview(
        StringHelper::Sprintf("###GyroPreview%s", id.c_str()).c_str(),
    ...
    ImGui::PushButtonRepeat(true);
    if (ImGui::Button(StringHelper::Sprintf("-##GyroSensitivity%s", id.c_str())
                          .c_str())) {
    ...
    ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
    if (ImGui::SliderInt(
            StringHelper::Sprintf("##GyroSensitivity%s", id.c_str()).c_str(),
            &sensitivity, 0, 100, "%d%%", ImGuiSliderFlags_AlwaysClamp)) {
    ...
    if (!mapping->SensitivityIsDefault()) {
      ImGui::SameLine();
      if (ImGui::Button(
              StringHelper::Sprintf("Reset to Default###resetGyroSensitivity%s",
                                    id.c_str())
                  .c_str())) {
...
void InitHeader(bool has_header = true) {
  ...
  ImGui::AlignTextToFramePadding(); // This is to adjust Vertical pos of item in
                                    // a cell to be normlized.
...
void PaperboatInputEditorWindow::addButtonName(N64ButtonMask mask,
                                               const char *name) {
  buttons.push_back(std::make_pair(mask, name));
  buttonNames[mask] = std::prev(buttons.end());
}

// Draw a button mapping setting consisting of a padded label and button
// dropdown. excludedButtons indicates which buttons are unavailable to choose
// from.
void PaperboatInputEditorWindow::DrawMapping(CustomButtonMap &mapping,
                                             float labelWidth,
                                             N64ButtonMask excludedButtons) {
  N64ButtonMask currentButton =
      CVarGetInteger(mapping.cVarName, mapping.defaultBtn);
  ...
  if (ImGui::BeginCombo(StringHelper::Sprintf("##%s", mapping.cVarName).c_str(),
                        preview)) {
...
void PaperboatInputEditorWindow::DrawDeviceToggles(uint8_t portIndex) {
  ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true);

  auto keyboardButtonColor = ImGui::GetStyleColorVec4(ImGuiCol_Button);
  auto keyboardButtonHoveredColor =
      ImGui::GetStyleColorVec4(ImGuiCol_ButtonHovered);
  GetButtonColorsForDeviceType(Ship::PhysicalDeviceType::Keyboard,
                               keyboardButtonColor, keyboardButtonHoveredColor);
  ...
  ImGui::Button(
      StringHelper::Sprintf("%s Keyboard", ICON_FA_KEYBOARD_O).c_str());
  ImGui::PopStyleColor();
  ImGui::PopStyleColor();

  auto mouseButtonColor = ImGui::GetStyleColorVec4(ImGuiCol_Button);
  auto mouseButtonHoveredColor =
      ImGui::GetStyleColorVec4(ImGuiCol_ButtonHovered);
  GetButtonColorsForDeviceType(Ship::PhysicalDeviceType::Mouse,
                               mouseButtonColor, mouseButtonHoveredColor);
  ...
  auto connectedDeviceManager = ControllerGetControlDeck()
                                    ->GetConnectedPhysicalDeviceManager();
  for (const auto &[instanceId, name] :
       connectedDeviceManager->GetConnectedSDLGamepadNames()) {
    ImGui::PushItemFlag(ImGuiItemFlags_Disabled, true);
    auto buttonColor = ImGui::GetStyleColorVec4(ImGuiCol_Button);
    auto buttonHoveredColor = ImGui::GetStyleColorVec4(ImGuiCol_ButtonHovered);
    GetButtonColorsForDeviceType(Ship::PhysicalDeviceType::SDLGamepad,
                                 buttonColor, buttonHoveredColor);
    ImGui::PushStyleColor(ImGuiCol_Button, buttonColor);
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, buttonHoveredColor);
    auto notIgnored = !connectedDeviceManager->PortIsIgnoringInstanceId(
        portIndex, instanceId);
    ImGui::PopItemFlag();
    if (ImGui::Checkbox(
            StringHelper::Sprintf("###instanceId_%d", instanceId).c_str(),
            &notIgnored)) {
      if (notIgnored) {
        connectedDeviceManager->UnignoreInstanceIdForPort(portIndex,
                                                          instanceId);
      } else {
    ...
    ImGui::SameLine();
    ImGui::Button(
        StringHelper::Sprintf("%s %s (SDL)", ICON_FA_GAMEPAD, name.c_str())
            .c_str());
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    ImGui::PopItemFlag();
...
void PaperboatInputEditorWindow::DrawClearAllButton(uint8_t portIndex) {
  if (ImGui::Button("Clear All", ImGui::CalcTextSize("Clear All") * 2)) {
    ImGui::OpenPopup("Clear All##clearAllPopup");
  }
  if (ImGui::BeginPopupModal("Clear All##clearAllPopup", NULL,
...
void PaperboatInputEditorWindow::DrawPortTab(uint8_t portIndex) {
  if (ImGui::BeginTabItem(
          StringHelper::Sprintf("Port %d###port%d", portIndex + 1, portIndex)
              .c_str())) {
...
void PaperboatInputEditorWindow::DrawPortTabContents(uint8_t portIndex) {
  ...
  if (ImGui::CollapsingHeader("Buttons", NULL,
    ...
    DrawButtonLine(StringHelper::Sprintf("C %s", ICON_FA_ARROW_UP).c_str(),
                   portIndex, BTN_CUP, CHIP_COLOR_N64_YELLOW);
    DrawButtonLine(StringHelper::Sprintf("C %s", ICON_FA_ARROW_DOWN).c_str(),
                   portIndex, BTN_CDOWN, CHIP_COLOR_N64_YELLOW);
    DrawButtonLine(StringHelper::Sprintf("C %s", ICON_FA_ARROW_LEFT).c_str(),
                   portIndex, BTN_CLEFT, CHIP_COLOR_N64_YELLOW);
    DrawButtonLine(StringHelper::Sprintf("C %s", ICON_FA_ARROW_RIGHT).c_str(),
                   portIndex, BTN_CRIGHT, CHIP_COLOR_N64_YELLOW);
  }

  if (ImGui::CollapsingHeader("D-Pad", NULL, ImGuiTreeNodeFlags_DefaultOpen)) {
    DrawButtonLine(StringHelper::Sprintf("%s##DPad", ICON_FA_ARROW_UP).c_str(),
                   portIndex, BTN_DUP);
    DrawButtonLine(
        StringHelper::Sprintf("%s##DPad", ICON_FA_ARROW_DOWN).c_str(),
        portIndex, BTN_DDOWN);
    DrawButtonLine(
        StringHelper::Sprintf("%s##DPad", ICON_FA_ARROW_LEFT).c_str(),
        portIndex, BTN_DLEFT);
    DrawButtonLine(
        StringHelper::Sprintf("%s##DPad", ICON_FA_ARROW_RIGHT).c_str(),
        portIndex, BTN_DRIGHT);
  }
...
void PaperboatInputEditorWindow::DrawSetDefaultsButton(uint8_t portIndex) {
  ImGui::SameLine();
  auto popupId = StringHelper::Sprintf("setDefaultsPopup##%d", portIndex);
  if (ImGui::Button(
          StringHelper::Sprintf("Set Defaults##%d", portIndex).c_str(),
  ...
  if (ImGui::BeginPopup(popupId.c_str())) {
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered,
                          BUTTON_COLOR_KEYBOARD_BEIGE_HOVERED);
    if (ImGui::Button(
            StringHelper::Sprintf("%s Keyboard", ICON_FA_KEYBOARD_O).c_str())) {
    ...
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    if (ImGui::BeginPopupModal("Set Defaults for Keyboard", NULL,
                               ImGuiWindowFlags_AlwaysAutoResize)) {
      ...
      if (ImGui::Button("Set defaults")) {
        ...
        ControllerGetControlDeck()
            ->GetControllerByPort(portIndex)
            ->AddDefaultMappings(Ship::PhysicalDeviceType::Keyboard);
        shouldClose = true;
        ImGui::CloseCurrentPopup();
      }
      ImGui::EndPopup();
    ...
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, buttonHoveredColor);
    if (ImGui::Button(
            StringHelper::Sprintf("%s %s", ICON_FA_GAMEPAD, "Gamepad (SDL)")
                .c_str())) {
      ImGui::OpenPopup("Set Defaults for Gamepad (SDL)");
    }
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    if (ImGui::BeginPopupModal("Set Defaults for Gamepad (SDL)", NULL,
                               ImGuiWindowFlags_AlwaysAutoResize)) {
      ImGui::Text("This will clear all existing mappings for\nGamepad (SDL) on "
                  "port %d.\n\nContinue?",
                  portIndex + 1);
...
void PaperboatInputEditorWindow::DrawFullContents() {
...
void PaperboatInputEditorWindow::DrawElement() { DrawFullContents(); }

void PaperboatInputEditorWindow::OffsetMappingPopup() {
