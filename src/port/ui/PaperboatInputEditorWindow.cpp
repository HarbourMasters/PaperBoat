...
void PaperboatInputEditorWindow::UpdateElement() {
    ...
    if (mInputEditorPopupOpen && ImGui::IsPopupOpen("", ImGuiPopupFlags_AnyPopupId)) {
        ControllerGetControlDeck()->BlockGameInput(INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
        ...
        WindowGetWindowComponent()->GetGui()->BlockGamepadNavigation();
    } else {
        if (mGameInputBlockTimer != INT32_MAX) {
            mGameInputBlockTimer--;
            if (mGameInputBlockTimer <= 0) {
                ControllerGetControlDeck()->UnblockGameInput(INPUT_EDITOR_WINDOW_GAME_INPUT_BLOCK_ID);
                mGameInputBlockTimer = INT32_MAX;
            }
        }

        if (WindowGetWindowComponent()->GetGui()->GamepadNavigationEnabled()) {
            mMappingInputBlockTimer = ImGui::GetIO().Framerate / 3;
        } else {
            mMappingInputBlockTimer = INT32_MAX;
        }

        WindowGetWindowComponent()->GetGui()->UnblockGamepadNavigation();
    }
}

void PaperboatInputEditorWindow::DrawAnalogPreview(const char* label, ImVec2 stick, float deadzone, bool gyro) {
    ImGui::BeginChild(label, ImVec2(gyro ? SCALE_IMGUI_SIZE(78) : SCALE_IMGUI_SIZE(96), SCALE_IMGUI_SIZE(85)), false);
    ImGui::SetCursorPos(ImVec2(
        ImGui::GetCursorPos().x + gyro ? SCALE_IMGUI_SIZE(10) : SCALE_IMGUI_SIZE(18),
    ...
    drawList->AddRect(
        ImVec2(borderSquareLeft, borderSquareTop),
        ImVec2(borderSquareLeft + borderSquareSize, borderSquareTop + borderSquareSize), ImColor(100, 100, 100, 255),
        0.0f, 0, 1.5f
    );
    ...
    ImVec2 joystickCenterpoint = ImVec2(
        cursorScreenPosition.x + cardinalRadius + SCALE_IMGUI_SIZE(12),
    ...
    drawList->AddCircleFilled(
        ImVec2(
            joystickCenterpoint.x + joystickIndicatorDistanceFromCenter.x,
...
void PaperboatInputEditorWindow::GetButtonColorsForDeviceType(
    Ship::PhysicalDeviceType lusIndex, ImVec4& buttonColor, ImVec4& buttonHoveredColor
) {
    switch (lusIndex) {
...
void PaperboatInputEditorWindow::DrawInputChip(const char* buttonName, ImVec4 color = CHIP_COLOR_N64_GREY) {
    ImGui::BeginDisabled();
...
void PaperboatInputEditorWindow::DrawButtonLineAddMappingButton(uint8_t port, N64ButtonMask bitmask) {
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
    auto popupId = StringHelper::Sprintf("addButtonMappingPopup##%d-%d", port, bitmask);
    if (ImGui::Button(
            StringHelper::Sprintf("%s###addButtonMappingButton%d-%d", ICON_FA_PLUS, port, bitmask).c_str(),
...
void PaperboatInputEditorWindow::DrawButtonLineEditMappingButton(uint8_t port, N64ButtonMask bitmask, std::string id) {
    auto mapping = ControllerGetControlDeck()->GetControllerByPort(port)->GetButton(bitmask)->GetButtonMappingById(id);
    if (mapping == nullptr) {
        return;
    }
    ...
    if (sdlAxisDirectionToButtonMapping != nullptr) {
        ...
        if (ImGui::Button(
                StringHelper::Sprintf("%s###editAxisThresholdButton%s", ICON_FA_COG, id.c_str()).c_str(),
        ...
        if (ImGui::BeginPopup(popupId.c_str())) {
            ...
            auto globalSettings = ControllerGetControlDeck()->GetGlobalSDLDeviceSettings();
            ...
            if (sdlAxisDirectionToButtonMapping->AxisIsStick()) {
                ...
                if (ImGui::SliderInt(
                        StringHelper::Sprintf("##Stick Axis Threshold%s", id.c_str()).c_str(), &stickAxisThreshold, 0,
            ...
            if (sdlAxisDirectionToButtonMapping->AxisIsTrigger()) {
                ...
                ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
                if (ImGui::SliderInt(
                        StringHelper::Sprintf("##Trigger Axis Threshold%s", id.c_str()).c_str(), &triggerAxisThreshold,
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
    if (ImGui::Button(
            StringHelper::Sprintf("%s###removeButtonMappingButton%s", ICON_FA_TIMES, id.c_str()).c_str(),
            ImVec2(ImGui::CalcTextSize(ICON_FA_TIMES).x + SCALE_IMGUI_SIZE(10.0f), 0.0f)
        ))
    {
        ControllerGetControlDeck()->GetControllerByPort(port)->GetButton(bitmask)->ClearButtonMapping(id);
    };
    ImGui::PopStyleColor();
    ImGui::PopStyleColor();
    ImGui::PopStyleVar();
...
void PaperboatInputEditorWindow::DrawButtonLine(
    const char* buttonName, uint8_t port, N64ButtonMask bitmask, ImVec4 color = CHIP_COLOR_N64_GREY
) {
    ImGui::NewLine();
...
void PaperboatInputEditorWindow::DrawStickDirectionLineEditMappingButton(
    ...
    if (stick == Ship::LEFT) {
        mapping = ControllerGetControlDeck()->GetControllerByPort(port)->GetLeftStick()->GetAxisDirectionMappingById(
            direction, id
        );
    } else {
        mapping = ControllerGetControlDeck()->GetControllerByPort(port)->GetRightStick()->GetAxisDirectionMappingById(
            direction, id
        );
    }
    ...
    if (ImGui::Button(
            StringHelper::Sprintf(
                "%s###editStickDirectionMappingButton%s", physicalInputDisplayName.c_str(), id.c_str()
            )
                .c_str(),
            ImVec2(ImGui::CalcTextSize(physicalInputDisplayName.c_str()).x + SCALE_IMGUI_SIZE(12.0f), 0.0f)
        ))
    ...
    {
        if (stick == Ship::LEFT) {
            ControllerGetControlDeck()->GetControllerByPort(port)->GetLeftStick()->ClearAxisDirectionMapping(
                direction, id
            );
        } else {
            ControllerGetControlDeck()->GetControllerByPort(port)->GetRightStick()->ClearAxisDirectionMapping(
                direction, id
            );
        }
    };
...
void PaperboatInputEditorWindow::DrawStickSection(uint8_t port, uint8_t stick, ImVec4 color = CHIP_COLOR_N64_GREY) {
    static int8_t sX, sY;
    std::shared_ptr<Ship::ControllerStick> controllerStick = nullptr;
    if (stick == Ship::LEFT) {
        controllerStick = ControllerGetControlDeck()->GetControllerByPort(port)->GetLeftStick();
    } else {
        controllerStick = ControllerGetControlDeck()->GetControllerByPort(port)->GetRightStick();
    }
    ...
    DrawStickDirectionLine(
        StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_UP, stick).c_str(), port, stick, Ship::UP, color
    );
    DrawStickDirectionLine(
        StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_DOWN, stick).c_str(), port, stick, Ship::DOWN, color
    );
    DrawStickDirectionLine(
        StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_LEFT, stick).c_str(), port, stick, Ship::LEFT, color
    );
    DrawStickDirectionLine(
        StringHelper::Sprintf("%s##%d", ICON_FA_ARROW_RIGHT, stick).c_str(), port, stick, Ship::RIGHT, color
    );
    ImGui::EndGroup();
    ...
    if (ImGui::TreeNode(StringHelper::Sprintf("Analog Stick Options##%d", stick).c_str())) {
        ...
        ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
        if (ImGui::SliderInt(
                StringHelper::Sprintf("##Sensitivity%d", stick).c_str(), &sensitivityPercentage, 0, 200, "%d%%",
        ...
        ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
        if (ImGui::SliderInt(
                StringHelper::Sprintf("##Deadzone%d", stick).c_str(), &deadzonePercentage, 0, 100, "%d%%",
                ImGuiSliderFlags_AlwaysClamp
            ))
        {
        ...
        ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
        if (ImGui::SliderInt(
                StringHelper::Sprintf("##NotchProximityThreshold%d", stick).c_str(), &notchSnapAngle, 0, 45, "%d°",
                ImGuiSliderFlags_AlwaysClamp
            ))
        {
...
void PaperboatInputEditorWindow::UpdateBitmaskToMappingIds(uint8_t port) {
    ...
    for (auto [bitmask, button] : ControllerGetControlDeck()->GetControllerByPort(port)->GetAllButtons()) {
...
void PaperboatInputEditorWindow::UpdateStickDirectionToMappingIds(uint8_t port) {
    // todo: do we need this?
    for (auto stick : { std::make_pair<uint8_t, std::shared_ptr<Ship::ControllerStick>>(
                            Ship::LEFT, ControllerGetControlDeck()->GetControllerByPort(port)->GetLeftStick()
                        ),
                        std::make_pair<uint8_t, std::shared_ptr<Ship::ControllerStick>>(
                            Ship::RIGHT, ControllerGetControlDeck()->GetControllerByPort(port)->GetRightStick()
                        ) })
    {
        for (auto direction : { Ship::LEFT, Ship::RIGHT, Ship::UP, Ship::DOWN }) {
            for (auto [id, mapping] : stick.second->GetAllAxisDirectionMappingByDirection(direction)) {
                ...
                // mappings to go to the end of the list instead of autosorting
                if (std::find(
                        mStickDirectionToMappingIds[port][stick.first][direction].begin(),
...
void PaperboatInputEditorWindow::DrawRemoveRumbleMappingButton(uint8_t port, std::string id) {
    ImGui::SameLine();
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
    if (ImGui::Button(
            StringHelper::Sprintf("%s###removeRumbleMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
            ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f))
        ))
    {
        ControllerGetControlDeck()->GetControllerByPort(port)->GetRumble()->ClearRumbleMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void PaperboatInputEditorWindow::DrawAddRumbleMappingButton(uint8_t port) {
    ...
    if (ImGui::Button(
            StringHelper::Sprintf("%s###addRumbleMapping%d", ICON_FA_PLUS, port).c_str(),
            ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f))
        ))
    {
        ImGui::OpenPopup(popupId.c_str());
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX
            && ControllerGetControlDeck()->GetControllerByPort(port)->GetRumble()->AddRumbleMappingFromRawPress())
        {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
...
void PaperboatInputEditorWindow::DrawRumbleSection(uint8_t port) {
    for (auto [id, mapping] :
         ControllerGetControlDeck()->GetControllerByPort(port)->GetRumble()->GetAllRumbleMappings())
    {
        ...
        ImGui::SameLine();
        if (ImGui::Button(
                StringHelper::Sprintf("%s###rumbleTestButton%s", TestingRumble() ? "Stop" : "Test", id.c_str()).c_str()
            ))
        {
            if (mRumbleTimer != INT32_MAX) {
                mRumbleTimer = INT32_MAX;
        ...
        if (open) {
            ...
            ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
            if (ImGui::SliderInt(
                    StringHelper::Sprintf("##Small Motor Intensity%s", id.c_str()).c_str(), &smallMotorIntensity, 0,
                    100, "%d%%", ImGuiSliderFlags_AlwaysClamp
                ))
            {
            ...
            if (!mapping->HighFrequencyIntensityIsDefault()) {
                ImGui::SameLine();
                if (ImGui::Button(
                        StringHelper::Sprintf("Reset to Default###resetHighFrequencyIntensity%s", id.c_str()).c_str()
                    ))
                {
            ...
            ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
            if (ImGui::SliderInt(
                    StringHelper::Sprintf("##Large Motor Intensity%s", id.c_str()).c_str(), &largeMotorIntensity, 0,
                    100, "%d%%", ImGuiSliderFlags_AlwaysClamp
                ))
            {
...
void PaperboatInputEditorWindow::DrawRemoveLEDMappingButton(uint8_t port, std::string id) {
    ImGui::SameLine();
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
    if (ImGui::Button(
            StringHelper::Sprintf("%s###removeLEDMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
            ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f))
        ))
    {
        ControllerGetControlDeck()->GetControllerByPort(port)->GetLED()->ClearLEDMapping(id);
    }
    ImGui::PopStyleVar();
}
...
void PaperboatInputEditorWindow::DrawAddLEDMappingButton(uint8_t port) {
    ...
    if (ImGui::Button(
            StringHelper::Sprintf("%s###addLEDMapping%d", ICON_FA_PLUS, port).c_str(),
            ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f))
        ))
    {
        ImGui::OpenPopup(popupId.c_str());
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX
            && ControllerGetControlDeck()->GetControllerByPort(port)->GetLED()->AddLEDMappingFromRawPress())
        {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
...
void PaperboatInputEditorWindow::DrawLEDSection(uint8_t port) {
    for (auto [id, mapping] : ControllerGetControlDeck()->GetControllerByPort(port)->GetLED()->GetAllLEDMappings()) {
        ImGui::AlignTextToFramePadding();
        ImGui::SetNextItemOpen(true, ImGuiCond_Once);
        ...
        if (open) {
            ...
            if (ImGui::Combo(
                    StringHelper::Sprintf("###ledColorSource%s", mapping->GetLEDMappingId().c_str()).c_str(),
            ...
            if (mapping->GetColorSource() == LED_COLOR_SOURCE_SET) {
                ...
                {
                    mapping->SetSavedColor(Color_RGB8(
                        { static_cast<uint8_t>(color.x * 255.0), static_cast<uint8_t>(color.y * 255.0),
...
void PaperboatInputEditorWindow::DrawRemoveGyroMappingButton(uint8_t port, std::string id) {
    ImGui::SameLine();
    ImGui::PushStyleVar(ImGuiStyleVar_ButtonTextAlign, ImVec2(1.0f, 0.5f));
    if (ImGui::Button(
            StringHelper::Sprintf("%s###removeGyroMapping%s", ICON_FA_TIMES, id.c_str()).c_str(),
            ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f))
        ))
    {
        ControllerGetControlDeck()->GetControllerByPort(port)->GetGyro()->ClearGyroMapping();
    }
    ImGui::PopStyleVar();
}
...
void PaperboatInputEditorWindow::DrawAddGyroMappingButton(uint8_t port) {
    ...
    if (ImGui::Button(
            StringHelper::Sprintf("%s###addGyroMapping%d", ICON_FA_PLUS, port).c_str(),
            ImVec2(SCALE_IMGUI_SIZE(20.0f), SCALE_IMGUI_SIZE(20.0f))
        ))
    {
        ImGui::OpenPopup(popupId.c_str());
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (mMappingInputBlockTimer == INT32_MAX
            && ControllerGetControlDeck()->GetControllerByPort(port)->GetGyro()->SetGyroMappingFromRawPress())
        {
            mInputEditorPopupOpen = false;
            ImGui::CloseCurrentPopup();
        }
...
void PaperboatInputEditorWindow::DrawGyroSection(uint8_t port) {
    auto mapping = ControllerGetControlDeck()->GetControllerByPort(port)->GetGyro()->GetGyroMapping();
    if (mapping != nullptr) {
        ...
        ImGui::SetNextItemWidth(SCALE_IMGUI_SIZE(160.0f));
        if (ImGui::SliderInt(
                StringHelper::Sprintf("##GyroSensitivity%s", id.c_str()).c_str(), &sensitivity, 0, 100, "%d%%",
                ImGuiSliderFlags_AlwaysClamp
            ))
        {
...
void PaperboatInputEditorWindow::addButtonName(N64ButtonMask mask, const char* name) {
...
// from.
void PaperboatInputEditorWindow::DrawMapping(
    CustomButtonMap& mapping, float labelWidth, N64ButtonMask excludedButtons
) {
...
void PaperboatInputEditorWindow::DrawDeviceToggles(uint8_t portIndex) {
    ...
    auto connectedDeviceManager = ControllerGetControlDeck()->GetConnectedPhysicalDeviceManager();
...
void PaperboatInputEditorWindow::DrawClearAllButton(uint8_t portIndex) {
    ...
    if (ImGui::BeginPopupModal("Clear All##clearAllPopup", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("Clear All")) {
            ControllerGetControlDeck()->GetControllerByPort(portIndex)->ClearAllMappings();
            ImGui::CloseCurrentPopup();
        }
        ImGui::EndPopup();
    }
...
void PaperboatInputEditorWindow::DrawPortTabContents(uint8_t portIndex) {
    ...
    if (ImGui::CollapsingHeader("Buttons", NULL, ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        DrawButtonLine(
            StringHelper::Sprintf("C %s", ICON_FA_ARROW_UP).c_str(), portIndex, BTN_CUP, CHIP_COLOR_N64_YELLOW
        );
        DrawButtonLine(
            StringHelper::Sprintf("C %s", ICON_FA_ARROW_DOWN).c_str(), portIndex, BTN_CDOWN, CHIP_COLOR_N64_YELLOW
        );
        DrawButtonLine(
            StringHelper::Sprintf("C %s", ICON_FA_ARROW_LEFT).c_str(), portIndex, BTN_CLEFT, CHIP_COLOR_N64_YELLOW
        );
        DrawButtonLine(
            StringHelper::Sprintf("C %s", ICON_FA_ARROW_RIGHT).c_str(), portIndex, BTN_CRIGHT, CHIP_COLOR_N64_YELLOW
        );
    }
    ...
    if (ImGui::BeginPopup(popupId.c_str())) {
        ...
        if (ImGui::BeginPopupModal("Set Defaults for Keyboard", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
            ...
            if (ImGui::Button("Set defaults")) {
                ControllerGetControlDeck()->GetControllerByPort(portIndex)->ClearAllMappingsForDeviceType(
                    Ship::PhysicalDeviceType::Keyboard
                );
                ControllerGetControlDeck()->GetControllerByPort(portIndex)->AddDefaultMappings(
                    Ship::PhysicalDeviceType::Keyboard
                );
                shouldClose = true;
                ImGui::CloseCurrentPopup();
...
void PaperboatInputEditorWindow::DrawElement() {
    DrawFullContents();
}
