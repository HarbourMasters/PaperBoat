...
static const std::unordered_map<int32_t, const char *> logLevels = {
    {DEBUG_LOG_TRACE, "Trace"}, {DEBUG_LOG_DEBUG, "Debug"},
    {DEBUG_LOG_INFO, "Info"},   {DEBUG_LOG_WARN, "Warn"},
    {DEBUG_LOG_ERROR, "Error"}, {DEBUG_LOG_CRITICAL, "Critical"},
    {DEBUG_LOG_OFF, "Off"},
};
...
void PaperboatMenu::AddMenuDevTools() {
  ...
  AddWidget(path, "Log Level", WIDGET_CVAR_COMBOBOX)
      ...
      .Options(ComboboxOptions()
                   .Tooltip("The log level determines which messages are "
  ...
  AddWidget(path, "Popout Console", WIDGET_WINDOW_BUTTON)
      ...
      .Options(WindowButtonOptions().Tooltip(
          "Enables the separate Console Window."));
