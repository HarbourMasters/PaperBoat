...
static std::map<uint32_t, float> notificationHeights =
    {}; // Cache actual heights by notification ID
...
void Window::Draw() {
  ...
  case 2: // Bottom Left
    ...
    break;
  case 3: // Bottom Right
    basePosition = ImVec2(vp->Pos.x + vp->Size.x - margin,
  ...
  for (int index = 0; index < notifications.size(); ++index) {
    ...
    if (index != 0) {
      ...
      if (it != notificationHeights.end()) {
        ...
        basePosition.y -=
            (notification.isAchievement ? 100.0f : 60.0f) + padding;
      }
    }

    if (notification.isAchievement) {
      // Enhanced layout for achievements
      DrawEnhancedNotification(notification, basePosition, position, padding,
                               index);
    } else {
      // Original simple layout for regular notifications
      DrawRegularNotification(notification, basePosition, inverseIndex,
                              position, padding, vp, index);
    }
  }
}

void Window::DrawRegularNotification(const Options &notification,
                                     ImVec2 basePosition, int inverseIndex,
                                     int position, float padding,
                                     ImGuiViewport *vp, int index) {
  ImGui::PushStyleColor(
      ImGuiCol_WindowBg,
      ImVec4(0, 0, 0, CVarGetFloat("gNotifications.BgOpacity", 0.5f)));
  ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(0, 0, 0, 0));
  ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 4.0f);
  ImGui::PushStyleVar(
      ImGuiStyleVar_ItemSpacing,
      ImVec2(8.0f * CVarGetFloat("gNotifications.Size", 1.8f), 6.0f));
  ImGui::PushStyleVar(
      ImGuiStyleVar_WindowPadding,
      ImVec2(8.0f * CVarGetFloat("gNotifications.Size", 1.8f), 8.0f));

  ImGui::SetNextWindowViewport(vp->ID);
  if (notification.remainingTime < 4.0f) {
    ImGui::PushStyleVar(ImGuiStyleVar_Alpha,
                        (notification.remainingTime - 1) / 3.0f);
  } else {
  ...
  case 3: // Bottom Right
    notificationPos = ImVec2(basePosition.x - currentWinSize.x,
                             basePosition.y - currentWinSize.y);
    break;
  }
  ...
  if (!notification.prefix.empty()) {
    ImGui::TextColored(notification.prefixColor, "%s",
                       notification.prefix.c_str());
    ImGui::SameLine();
  }
  ImGui::TextColored(notification.messageColor, "%s",
                     notification.message.c_str());
  if (!notification.suffix.empty()) {
    ImGui::SameLine();
    ImGui::TextColored(notification.suffixColor, "%s",
                       notification.suffix.c_str());
  }
...
void Window::DrawEnhancedNotification(const Options &notification,
                                      ImVec2 basePosition, int position,
                                      float padding, int index) {
  const float enhancedWidth = 380.0f;
  const float iconSize = 54.0f;
  const float textAreaWidth =
      enhancedWidth - iconSize - 30.0f; // Account for padding and spacing
  ...
  ImGui::PushStyleColor(
      ImGuiCol_WindowBg,
      ImVec4(0.08f, 0.08f, 0.12f, 0.95f)); // Darker, more opaque
  ImGui::PushStyleColor(ImGuiCol_Border,
                        ImVec4(1.0f, 0.85f, 0.0f, 1.0f)); // Gold border
  ...
  ImGui::SetNextWindowSize(
      ImVec2(enhancedWidth, 0)); // Auto-height with fixed width

  std::string windowName =
      "enhanced_notification#" + std::to_string(notification.id);
...
void EmitAchievement(const char *iconPath, const std::string &achievementName,
                     int harbourMastery) {
...
void EmitAchievementProgress(const char *iconPath, const char *name,
                             int current, int target) {
  char progressText[128];
  snprintf(progressText, sizeof(progressText), "%s: %d / %d", name, current,
           target);
...
void EmitAchievementProgressWithEvent(const char *iconPath,
                                      const char *eventName,
                                      const char *achievementName, int current,
                                      int target) {
  char progressText[128];
  snprintf(progressText, sizeof(progressText), "%s\n%s: %d / %d", eventName,
           achievementName, current, target);
...
bool IsNotificationActive() { return !notifications.empty(); }
