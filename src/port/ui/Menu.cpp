...
void Menu::InsertSidebarSearch() {
  ...
  uint32_t curIndex = 0;
  if (!Ship_IsCStringEmpty(
          CVarGetString(menuEntries["Settings"].sidebarCvar, ""))) {
    curIndex = GetVectorIndexOf(
        menuEntries["Settings"].sidebarOrder,
        CVarGetString(menuEntries["Settings"].sidebarCvar, ""));
  }
  menuEntries["Settings"].sidebarOrder.insert(
      menuEntries["Settings"].sidebarOrder.begin() + searchSidebarIndex,
      "Search");
  if (curIndex > searchSidebarIndex) {
    CVarSetString(menuEntries["Settings"].sidebarCvar,
                  menuEntries["Settings"].sidebarOrder.at(curIndex).c_str());
  }
}
...
void Menu::RemoveSidebarSearch() {
  ...
  std::erase_if(menuEntries["Settings"].sidebarOrder,
...
void Menu::UpdateWindowBackendObjects() {
  ...
  if (Ship::Context::GetInstance()->GetWindow()->IsAvailableWindowBackend(
    ...
    configWindowBackend =
        static_cast<Ship::WindowBackend>(configWindowBackendId);
  } else {
    configWindowBackend = runningWindowBackend;
  }

  availableWindowBackends =
      Ship::Context::GetInstance()->GetWindow()->GetAvailableWindowBackends();
  for (auto &backend : *availableWindowBackends) {
    availableWindowBackendsMap[backend] = windowBackendsMap.at(backend);
  }
}

bool Menu::IsMenuPopped() { return popped; }

UIWidgets::Colors Menu::GetMenuThemeColor() { return menuThemeIndex; }

Menu::Menu(const std::string &cVar, const std::string &name,
           uint8_t searchSidebarIndex_, UIWidgets::Colors defaultThemeIndex_)
...
void Menu::InitElement() {
  ...
  menuThemeIndex = static_cast<UIWidgets::Colors>(
      CVarGetInteger(CVAR_SETTING("Menu.Theme"), defaultThemeIndex));
...
bool ModernMenuSidebarEntry(std::string label) {
  ...
  const ImGuiID sidebarId =
      window->GetID(std::string(label + "##Sidebar").c_str());
  ImVec2 labelSize = ImGui::CalcTextSize(
      label.c_str(), ImGui::FindRenderedTextEnd(label.c_str()), true);
  ...
  window->DrawList->AddRectFilled(
      ...
      ImGui::GetColorU32((held && hovered) ? ImGuiCol_ButtonActive
                         : hovered         ? ImGuiCol_ButtonHovered
                                           : ImGuiCol_Button),
      3.0f);
  UIWidgets::RenderText(pos, label.c_str(),
                        ImGui::FindRenderedTextEnd(label.c_str()), true);
  return pressed;
}
...
bool ModernMenuHeaderEntry(std::string label) {
  ...
  ImGuiStyle &style = ImGui::GetStyle();
  ImVec2 pos = window->DC.CursorPos;
  const ImGuiID headerId =
      window->GetID(std::string(label + "##Header").c_str());
  ImVec2 labelSize = ImGui::CalcTextSize(
      label.c_str(), ImGui::FindRenderedTextEnd(label.c_str()), true);
...
uint32_t Menu::DrawSearchResults(std::string &menuSearchText) {
  int searchCount = 0;
  std::transform(menuSearchText.begin(), menuSearchText.end(),
                 menuSearchText.begin(), ::tolower);
  menuSearchText.erase(
      std::remove(menuSearchText.begin(), menuSearchText.end(), ' '),
      menuSearchText.end());
  ImGui::SetNextWindowSizeConstraints(
      {ImGui::GetContentRegionAvail().x / 2, 0},
      {ImGui::GetContentRegionAvail().x / 2, ImGui::GetContentRegionAvail().y});
  if (ImGui::BeginChild(
          "Search Results Col 1", {ImGui::GetContentRegionAvail().x / 2, 0},
      ...
      for (auto &sidebarLabel : menuEntry.sidebarOrder) {
        ...
        for (int i = 0; i < sidebar.columnWidgets.size(); i++) {
          ...
          for (auto &info : column) {
            ...
            std::string widgetStr = std::string(info.name) +
                                    std::string(tooltip != NULL ? tooltip : "");
            std::transform(widgetStr.begin(), widgetStr.end(),
                           widgetStr.begin(), ::tolower);
            widgetStr.erase(
                std::remove(widgetStr.begin(), widgetStr.end(), ' '),
                widgetStr.end());
            if (widgetStr.find(menuSearchText) != std::string::npos) {
              ...
              if (info.type == WIDGET_COMBOBOX ||
                  info.type == WIDGET_CVAR_COMBOBOX) {
                backupAlignment =
                    std::static_pointer_cast<UIWidgets::ComboboxOptions>(
                        info.options)
                        ->alignment;
                backupLabelPos =
                    std::static_pointer_cast<UIWidgets::ComboboxOptions>(
                ...
                std::static_pointer_cast<UIWidgets::ComboboxOptions>(
                    info.options)
                    ->alignment = UIWidgets::ComponentAlignments::Left;
                std::static_pointer_cast<UIWidgets::ComboboxOptions>(
                    info.options)
              ...
              std::string origin = fmt::format(
                  "  ({} -> {}, Col {})", menuEntry.label, sidebarLabel, i + 1);
              ...
              if (info.type == WIDGET_COMBOBOX ||
                ...
                std::static_pointer_cast<UIWidgets::ComboboxOptions>(
                    info.options)
                    ->alignment = backupAlignment;
                std::static_pointer_cast<UIWidgets::ComboboxOptions>(
                    info.options)
                    ->labelPosition = backupLabelPos;
              }
            }
          }
        }
    ...
    for (auto &entry : extraSearchWidgets) {
      if (entry.info.type == WIDGET_SEARCH ||
          entry.info.type == WIDGET_SEPARATOR ||
          entry.info.type == WIDGET_SEPARATOR_TEXT || entry.info.isHidden ||
          entry.info.hideInSearch) {
        continue;
      }
      std::string widgetStr = entry.info.name + entry.info.options->tooltip +
                              entry.extraTerms + entry.sidebarName;
      std::transform(widgetStr.begin(), widgetStr.end(), widgetStr.begin(),
                     ::tolower);
      widgetStr.erase(std::remove(widgetStr.begin(), widgetStr.end(), ' '),
                      widgetStr.end());
      if (widgetStr.find(menuSearchText) != std::string::npos) {
        MenuDrawItem(entry.info, 400, menuThemeIndex);
        ImGui::PushStyleColor(
            ImGuiCol_Text, UIWidgets::ColorValues.at(UIWidgets::Colors::Gray));
        std::string origin = fmt::format("  ({} -> {}, {})", entry.menuName,
                                         entry.sidebarName, entry.location);
        ImGui::Text("%s", origin.c_str());
        ImGui::PopStyleColor();
        searchCount++;
      }
...
void Menu::MenuDrawItem(WidgetInfo &widget, uint32_t width,
  ...
  try {
    ...
    case WIDGET_CHECKBOX: {
      ...
      if (pointer == nullptr) {
        SPDLOG_ERROR(
            "Checkbox Widget requires a value pointer, currently nullptr");
        assert(false);
        return;
      }
      auto options =
          std::static_pointer_cast<UIWidgets::CheckboxOptions>(widget.options);
      options->color = menuThemeIndex;
      if (UIWidgets::Checkbox(
              UIWidgets::WrappedText(widget.name.c_str(), width).c_str(),
              pointer, *options)) {
    ...
    case WIDGET_VIDEO_BACKEND: {
      ...
      options.disabledTooltip =
          "Only one renderer API is available on this platform.";
    ...
    case WIDGET_SEPARATOR_TEXT: {
      auto options =
          std::static_pointer_cast<UIWidgets::TextOptions>(widget.options);
      if (options->color != UIWidgets::Colors::NoColor) {
        ImGui::PushStyleColor(ImGuiCol_Text,
                              UIWidgets::ColorValues.at(options->color));
      }
    ...
    case WIDGET_COMBOBOX: {
      ...
      if (pointer == nullptr) {
        SPDLOG_ERROR(
            "Combobox Widget requires a value pointer, currently nullptr");
        assert(false);
        return;
      }
      auto options =
          std::static_pointer_cast<UIWidgets::ComboboxOptions>(widget.options);
      options->color = menuThemeIndex;
      if (UIWidgets::Combobox(widget.name.c_str(), pointer, options->comboMap,
                              *options)) {
        if (widget.callback != nullptr) {
          widget.callback(widget);
        }
    ...
    case WIDGET_CVAR_COMBOBOX: {
      auto options =
          std::static_pointer_cast<UIWidgets::ComboboxOptions>(widget.options);
      options->color = menuThemeIndex;
      if (UIWidgets::CVarCombobox(widget.name.c_str(), widget.cVar,
                                  options->comboMap, *options)) {
        if (widget.callback != nullptr) {
          widget.callback(widget);
        }
      }
    ...
    case WIDGET_SLIDER_INT: {
      ...
      if (pointer == nullptr) {
        SPDLOG_ERROR(
            "int32 Slider Widget requires a value pointer, currently nullptr");
        assert(false);
        return;
      }
      auto options =
          std::static_pointer_cast<UIWidgets::IntSliderOptions>(widget.options);
      options->color = menuThemeIndex;
    ...
    case WIDGET_SLIDER_FLOAT: {
      ...
      if (pointer == nullptr) {
        SPDLOG_ERROR(
            "float Slider Widget requires a value pointer, currently nullptr");
        assert(false);
        return;
      }
      auto options = std::static_pointer_cast<UIWidgets::FloatSliderOptions>(
          widget.options);
      options->color = menuThemeIndex;
    ...
    case WIDGET_BUTTON: {
      auto options =
          std::static_pointer_cast<UIWidgets::ButtonOptions>(widget.options);
      options->color = menuThemeIndex;
    ...
    case WIDGET_WINDOW_BUTTON: {
      if (widget.windowName == nullptr || widget.windowName[0] == '\0') {
        std::string msg = fmt::format(
            "Error drawing window contents for {}: windowName not defined",
            widget.name);
      ...
      if (!window) {
        std::string msg = fmt::format(
            "Error drawing window contents: windowName {} does not exist",
            widget.windowName);
        SPDLOG_ERROR(msg.c_str());
        break;
      }
      auto options = std::static_pointer_cast<UIWidgets::WindowButtonOptions>(
          widget.options);
      options->color = menuThemeIndex;
    ...
    case WIDGET_CVAR_COLOR_PICKER: {
      auto options = std::static_pointer_cast<UIWidgets::ColorPickerOptions>(
          widget.options);
      ...
      UIWidgets::CVarColorPicker(widget.name.c_str(), widget.cVar,
                                 options->defaultValue, options->useAlpha,
    ...
    case WIDGET_SEARCH: {
      ...
      ImGui::PushStyleColor(ImGuiCol_Border,
                            UIWidgets::ColorValues.at(menuThemeIndex));
      menuSearch.Draw();
      ImGui::PopStyleColor();
    ...
    SPDLOG_ERROR("Failed to draw menu item \"{}\" due to: {}", widget.name,
                 e.what());
    assert(false);
  }
}
...
void Menu::DrawElement() {
  ...
  if (!popout) {
    ImGui::SetNextWindowSize(
        {static_cast<float>(windowWidth), static_cast<float>(windowHeight)},
        windowCond);
    ImGui::SetNextWindowPos(ImGui::GetMainViewport()->GetCenter(), windowCond,
                            {0.5f, 0.5f});
  ...
  ImGui::PushStyleColor(
      ImGuiCol_WindowBg,
      ImVec4(0, 0, 0,
             CVarGetFloat(CVAR_SETTING("Menu.BackgroundOpacity"), 0.85f)));
  ...
  float centerX = pos.x + windowWidth / 2 -
                  (style.ItemSpacing.x * (menuEntries.size() + 1));
  ...
  ImVec2 buttonSize =
      ImGui::CalcTextSize(ICON_FA_TIMES_CIRCLE) + style.FramePadding * 2;
  bool scrollbar = false;
  if (headerWidth > menuSize.x - buttonSize.x * 3 - style.ItemSpacing.x * 3) {
    headerHeight += style.ScrollbarSize;
    scrollbar = true;
  }
  ImGui::SetNextWindowSizeConstraints({0, headerHeight},
                                      {headerWidth, headerHeight});
  ImVec2 headerSelSize = {
      menuSize.x - buttonSize.x * 3 - style.ItemSpacing.x * 3, headerHeight};
  if (scrollbar) {
  ...
  if (UIWidgets::Button(ICON_FA_POWER_OFF, options3)) {
    PaperboatGui::mModalWindow->RegisterPopup(
        "Quit Paperboat", "Are you sure you want to quit Paperboat?", "Quit",
        "Cancel",
        []() {
  ...
  pos.x = centerX - menuSize.x / 2 +
          (style.ItemSpacing.x * (menuEntries.size() + 1));
  window->DrawList->AddRectFilled(pos, pos + ImVec2{menuSize.x, 4},
  ...
  float topY = pos.y;
  ImGui::SetNextWindowSizeConstraints({sidebarWidth, 0},
                                      {sidebarWidth, columnHeight});
  ImGui::BeginChild((menuEntries.at(headerIndex).label + " Section").c_str(),
  ...
  pos =
      ImVec2{sectionCenterX + (sidebarWidth / 2), topY} + style.ItemSpacing * 2;
  window->DrawList->AddRectFilled(
  ...
  if (!useColumns || (headerSearch && menuSearchText.length() > 0)) {
    ImGui::SameLine();
    ImGui::SetNextWindowSizeConstraints({sectionWidth, 0},
                                        {sectionWidth, columnHeight});
  ...
  if (headerSearch && menuSearchText.length() > 0) {
    ...
    if (searchCount == 0) {
      ImGui::SetCursorPosX((ImGui::GetWindowWidth() -
                            ImGui::CalcTextSize("No results found").x) /
                           2);
    ...
    if (MenuInit::GetUpdateFuncs().contains(menuLabel)) {
      if (MenuInit::GetUpdateFuncs()[menuLabel].contains(sectionIndex)) {
        for (auto &updateFunc :
             MenuInit::GetUpdateFuncs()[menuLabel][sectionIndex]) {
          updateFunc();
        }
      }
    }
    for (int i = 0; i < columnFuncs; i++) {
      ...
      if (useColumns) {
        ImGui::SetNextWindowSizeConstraints({columnWidth, 0},
