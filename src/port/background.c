/*
 * Speaks the per-asset OTR layout (separate CI8 raster and RGBA16 palette with metadata-driven
 * dimensions) instead of parsing a monolithic blob with a packed header.
 */
...
void port_load_map_bg(char* optAssetName) {
    ...
    gBackgroundImage.width = ResourceGetBaseTexWidthByName(sBgRasterPath);
    gBackgroundImage.height = ResourceGetBaseTexHeightByName(sBgRasterPath);
...
void port_appendGfx_background_texture(void) {
    ...
    // Palette: non-fog passes OTR path (interpreter detects + loads alt),
    // fog passes CPU-modified palette data directly.
    if (!(gGameStatusPtr->backgroundFlags & BACKGROUND_FLAG_FOG)) {
    ...
    gDPLoadTextureTile(gMainGfxPos++, gGameStatusPtr->backgroundRaster,
                       G_IM_FMT_CI, G_IM_SIZ_8b, bgMaxX, bgMaxY,
                       0, 0, bgMaxX - 1, bgMaxY - 1, 0,
                       G_TX_WRAP, G_TX_WRAP, G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);

    if (!gBackroundWaveEnabled) {
        // Two rectangles for horizontal scroll wrapping
        gSPTextureRectangle(gMainGfxPos++, bgMinX * 4, bgMinY * 4,
                                             (bgXOffset + bgMinX - 1) * 4, (bgMaxY - 1 + bgMinY) * 4,
                                             G_TX_RENDERTILE, (bgMaxX - bgXOffset) * 32, 0, 4096, 1024);
        gSPTextureRectangle(gMainGfxPos++, (bgXOffset + bgMinX) * 4, bgMinY * 4,
        ...
        lineHeight = 6;
        numLines = SCREEN_HEIGHT / lineHeight;
        extraHeight = SCREEN_HEIGHT % lineHeight;
        ...
        for (i = 0; i < numLines; i++) {
            ...
            gSPTextureRectangle(gMainGfxPos++, bgMinX * 4, (lineHeight * i + bgMinY) * 4,
                                                 ...
                                                 G_TX_RENDERTILE, bgMaxX * 32 - bgXOffset * 16, (lineHeight * i) * 32, 4096, 1024);
        ...
        if (extraHeight != 0) {
            ...
            bgXOffset = 2.0f * (gGameStatusPtr->backgroundXOffset + waveOffset);
            gSPTextureRectangle(gMainGfxPos++, bgMinX * 4, (lineHeight * i + bgMinY) * 4,
                                                 (2 * bgXOffset + (bgMinX - 1)) * 4, (SCREEN_HEIGHT - 1 + bgMinY) * 4,
                                                 G_TX_RENDERTILE, bgMaxX * 32 - bgXOffset * 16, (lineHeight * i) * 32, 4096, 1024);
            gSPTextureRectangle(gMainGfxPos++, bgXOffset * 2 + bgMinX * 4, (lineHeight * i + bgMinY) * 4,
                                                 (bgMaxX + bgMinX - 1) * 4, (SCREEN_HEIGHT - 1 + bgMinY) * 4,
                                                 G_TX_RENDERTILE, 0, (lineHeight * i) * 32, 4096, 1024);
        }
    }
}
