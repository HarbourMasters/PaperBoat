...
SpriteS32 Sprite_AssetsAvailable(void) { return sSpritesAvailable; }
...
size_t Sprite_GetNPCSize(SpriteS32 spriteIdx) {
  ...
  size_t nativeSize = ConvertN64SpriteToNative(
      reinterpret_cast<const uint8_t *>(spriteData), blobSize, nullptr, 0,
      false /* isPlayerSprite */, nullptr /* assetPath */);
...
void *Sprite_LoadNPC(SpriteS32 spriteIdx, void *destBuffer, size_t bufferSize) {
  ...
  if (spriteData == nullptr) {
    SPDLOG_WARN("Sprite_LoadNPC: Failed to load sprite {} from {}", spriteIdx,
                assetPath);
    return nullptr;
  }
  ...
  if (blobSize == 0) {
    SPDLOG_WARN(
        "Sprite_LoadNPC: Could not determine blob size for NPC sprite {}",
        spriteIdx);
    return nullptr;
  }
...
SpriteS32 Sprite_GetDataHeader(int32_t *outHeader) {
  ...
  if (sSpritesAvailable &&
      (sSpriteDataHeader[0] != 0 || sSpriteDataHeader[1] != 0 ||
       sSpriteDataHeader[2] != 0)) {
...
SpriteS32 Sprite_GetPlayerRasterHeader(int32_t *outHeader) {
  ...
  if (headerData == nullptr) {
    SPDLOG_WARN(
        "Sprite_GetPlayerRasterHeader: Failed to load player_raster_header");
    return 0;
  }
...
SpriteS32 Sprite_GetPlayerRasterSets(int32_t *outSets, SpriteS32 maxCount) {
  ...
  if (setsData == nullptr) {
    SPDLOG_WARN(
        "Sprite_GetPlayerRasterSets: Failed to load player_raster_sets");
    return 0;
  }
...
SpriteS32 Sprite_GetPlayerSpriteIndexEntry(SpriteS32 spriteIdx,
                                           int32_t *outEntry) {
  if (outEntry == nullptr) {
    return 0;
  }
  ...
  if (indexData == nullptr) {
    SPDLOG_WARN(
        "Sprite_GetPlayerSpriteIndexEntry: Failed to load player_sprite_index");
    return 0;
  }
...
static size_t ConvertN64SpriteToNative(const uint8_t *srcBlob, size_t srcSize,
  ...
  const N64_SpriteAnimData *n64Header =
      reinterpret_cast<const N64_SpriteAnimData *>(srcBlob);
  ...
  const uint32_t *animOffsets =
      reinterpret_cast<const uint32_t *>(srcBlob + 0x10);
  ...
  size_t headerSize = sizeof(void *) * 2 + sizeof(int32_t) * 2 +
                      sizeof(void *) * (numAnims + 1);
  ...
  size_t totalSize = headerSize + rasterArraySize + rasterEntriesSize +
                     paletteArraySize + compListArraysSize + compEntriesSize +
                     srcSize;
  ...
  if (destBufferSize < totalSize) {
    SPDLOG_ERROR("Dest buffer too small: need {}, have {}", totalSize,
                 destBufferSize);
    return 0;
  }
  ...
  size_t headerPtrCount =
      2 + (numAnims + 1); // rastersOffset, palettesOffset, animListStart[]
  int32_t *headerIntPtr =
      reinterpret_cast<int32_t *>(writePtr + sizeof(void *) * 2);

  writePtr += sizeof(void *) * 2 + sizeof(int32_t) * 2 +
              sizeof(void *) * (numAnims + 1);
  ...
  NativeRasterEntry *nativeRasterEntries =
      reinterpret_cast<NativeRasterEntry *>(writePtr);
  ...
  void ***animListStart = reinterpret_cast<void ***>(
      reinterpret_cast<uint8_t *>(headerIntPtr) + sizeof(int32_t) * 2);
  ...
  // size
  const uint32_t *n64RasterOffsets =
      reinterpret_cast<const uint32_t *>(srcBlob + n64Header->rastersOffset);
  ...
  const uint32_t *n64PaletteOffsets =
      reinterpret_cast<const uint32_t *>(srcBlob + n64Header->palettesOffset);
  ...
  for (int animIdx = 0; animIdx < numAnims; animIdx++) {
    ...
    const uint32_t *n64CompOffsets =
        reinterpret_cast<const uint32_t *>(srcBlob + animOffset);
    int numComps = CountN64PtrArray(srcBlob, animOffset);
    ...
    for (int compIdx = 0; compIdx < numComps; compIdx++) {
      ...
      const N64_SpriteAnimComponent *n64Comp =
          reinterpret_cast<const N64_SpriteAnimComponent *>(srcBlob +
                                                            compOffset);
      ...
      NativeComp *nativeComp =
          reinterpret_cast<NativeComp *>(currentCompEntryPtr);
...
size_t Sprite_GetPlayerSize(SpriteS32 spriteIdx) {
  ...
  size_t nativeSize = ConvertN64SpriteToNative(
      reinterpret_cast<const uint8_t *>(spriteData), blobSize, nullptr, 0,
      true /* isPlayerSprite */, nullptr /* assetPath */);

  return nativeSize;
}

void *Sprite_LoadPlayer(SpriteS32 spriteIdx, void *destBuffer,
                        size_t bufferSize) {
  if (destBuffer == nullptr || bufferSize == 0) {
  ...
  if (spriteData == nullptr) {
    SPDLOG_WARN("Sprite_LoadPlayer: Failed to load player sprite {} from {}",
                spriteIdx, assetPath);
    return nullptr;
  }
  ...
  if (blobSize == 0) {
    SPDLOG_WARN(
        "Sprite_LoadPlayer: Could not determine blob size for player sprite {}",
        spriteIdx);
    return nullptr;
  }
  ...
  if (convertedSize == 0) {
    SPDLOG_ERROR("Sprite_LoadPlayer: Failed to convert player sprite {}",
                 spriteIdx);
    return nullptr;
  }
...
SpriteS32 Sprite_GetPlayerRasterLoadDescriptors(SpriteS32 spriteIdx,
                                                SpriteS32 startIndex,
                                                int32_t *outBuffer,
                                                SpriteS32 count) {
  ...
  if (descData == nullptr) {
    SPDLOG_WARN("Sprite_GetPlayerRasterLoadDescriptors: Failed to load "
...
SpriteS32 Sprite_LoadPlayerRaster(SpriteS32 rasterOffset, void *destBuffer,
                                  SpriteS32 size) {
  if (destBuffer == nullptr || size <= 0) {
    SPDLOG_ERROR(
        "Sprite_LoadPlayerRaster: Invalid params - destBuffer={} size={}",
        destBuffer, size);
    return 0;
  }
  ...
  if (rasterOffset < 0) {
    SPDLOG_ERROR("Sprite_LoadPlayerRaster: Negative offset 0x{:X} ({})",
                 (uint32_t)rasterOffset, rasterOffset);
    return 0;
  }
  ...
  if (imageData == nullptr) {
    SPDLOG_WARN(
        "Sprite_LoadPlayerRaster: Failed to load player_raster_image_data");
    return 0;
  }
  ...
  if ((size_t)(rasterOffset + size) > PLAYER_RASTER_BLOB_SIZE) {
    SPDLOG_ERROR("Sprite_LoadPlayerRaster: Out of bounds! offset=0x{:X} "
                 "size={} (end=0x{:X}, max=0x{:X})",
                 rasterOffset, size, rasterOffset + size,
                 PLAYER_RASTER_BLOB_SIZE);
    return 0;
  }
