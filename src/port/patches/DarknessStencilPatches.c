...
void port_appendGfx_darkness_stencil(b32 isWorld, s32 posX, s32 posY, f32 alpha, f32 progress) {
    ...
    envA  = progress - primA;
    if (primA < 0.0f) primA = 0.0f;
    if (primA > 255.0f) primA = 255.0f;
    if (envA  < 0.0f) envA  = 0.0f;
    if (envA  > 255.0f) envA  = 255.0f;
    ...
    gDPLoadTextureTile(gMainGfxPos++, sDarknessStencil,
                       G_IM_FMT_I, G_IM_SIZ_8b, DARKNESS_STENCIL_W, DARKNESS_STENCIL_W,
                       0, 0, DARKNESS_STENCIL_W - 1, DARKNESS_STENCIL_W - 1, 0,
    ...
    gDPSetCombineLERP(gMainGfxPos++,
                      0, 0, 0, 0,    1, TEXEL0, ENVIRONMENT, PRIMITIVE,
                      0, 0, 0, 0,    1, TEXEL0, ENVIRONMENT, PRIMITIVE);
