...
static GlyphSheet sSheets[] = {
    { (const u8*)MsgCharImgNormal,   0x5100, 128, "__OTR__charset/charset_standard_", sStandardPaths, ARRAY_COUNT(sStandardPaths) },
    { (const u8*)MsgCharImgTitle,    0xF60,  96,  "__OTR__charset/charset_title_",    sTitlePaths,    ARRAY_COUNT(sTitlePaths) },
    { (const u8*)MsgCharImgSubtitle, 0xB88,  72,  "__OTR__charset/charset_subtitle_", sSubtitlePaths, ARRAY_COUNT(sSubtitlePaths) },
};
...
void port_msg_font_loaded(s32 font) {
    ...
    sPalettePrefix = font == 1 ? "__OTR__charset/charset_subtitle_palette_" : "__OTR__charset/charset_standard_palette_";
}
