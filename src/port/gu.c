...
void guMtxIdentF(float mf[4][4]) { guIdentityF(mf); }
...
#define THRESH2 0.1f
  ...
  if (len > THRESH2) {
    ...
    h->h.x1 = twidth * 4 +
              (int)((xHilite * xRight + yHilite * yRight + zHilite * zRight) *
                    twidth * 2);
    h->h.y1 =
        theight * 4 +
...
void guFrustumF(float mf[4][4], float l, float r, float b, float t, float n,
                float f, float scale) {
  guIdentityF(mf);
  mf[0][0] = (2.0f * n) / (r - l);
  mf[1][1] = (2.0f * n) / (t - b);
