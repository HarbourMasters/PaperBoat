...
extern void transform_point(Matrix4f mtx, f32 inX, f32 inY, f32 inZ, f32 inS,
                            f32 *outX, f32 *outY, f32 *outZ, f32 *outW);
extern void draw_prev_frame_buffer_at_screen_pos(s32 x1, s32 y1, s32 x2, s32 y2,
                                                 f32 alpha);
// draw_box declaration now comes from functions.h (uses void* for windowStyle
// on PC)
extern void draw_msg(intptr_t msgID, s32 posX, s32 posY, s32 opacity,
...
void shim_guPerspectiveF(f32 mf[4][4], u16 *perspNorm, f32 fovy, f32 aspect,
                         f32 near, f32 far, f32 scale) {
  guPerspectiveF(mf, perspNorm, fovy, aspect, near, far, scale);
}

void shim_guPositionF(float mf[4][4], float r, float p, float h, float s,
                      float x, float y, float z) {
  guPositionF(mf, r, p, h, s, x, y, z);
}

void shim_guOrthoF(float mf[4][4], float l, float r, float b, float t, float n,
                   float f, float scale) {
  guOrthoF(mf, l, r, b, t, n, f, scale);
}

void shim_guFrustumF(float mf[4][4], float l, float r, float b, float t,
                     float n, float f, float scale) {
...
bool shim_npc_raycast_down_sides(s32 ignoreFlags, f32 *posX, f32 *posY,
                                 f32 *posZ, f32 *hitDepth) {
...
void shim_sfx_play_sound_at_position(s32 soundID, s32 value2, f32 posX,
                                     f32 posY, f32 posZ) {
...
bool shim_is_point_visible(f32 x, f32 y, f32 z, s32 depthQueryID, f32 *screenX,
                           f32 *screenY) {
  return is_point_visible(x, y, z, depthQueryID, screenX, screenY);
}

void shim_transform_point(Matrix4f mtx, f32 inX, f32 inY, f32 inZ, f32 inS,
                          f32 *outX, f32 *outY, f32 *outZ, f32 *outW) {
  transform_point(mtx, inX, inY, inZ, inS, outX, outY, outZ, outW);
}

void shim_draw_prev_frame_buffer_at_screen_pos(s32 x1, s32 y1, s32 x2, s32 y2,
                                               f32 alpha) {
  draw_prev_frame_buffer_at_screen_pos(x1, y1, x2, y2, alpha);
}

void shim_draw_box(s32 flags, void *windowStyle, s32 posX, s32 posY, s32 posZ,
                   s32 width, s32 height, u8 opacity, u8 darkening, f32 scaleX,
                   f32 scaleY, f32 rotX, f32 rotY, f32 rotZ,
                   void (*fpDrawContents)(void *), void *drawContentsArg0,
  ...
  draw_box(flags, windowStyle, posX, posY, posZ, width, height, opacity,
           darkening, scaleX, scaleY, rotX, rotY, rotZ, NULL, drawContentsArg0,
           rotScaleMtx, translateX, translateY, *outPtr);
}

void shim_draw_msg(s32 msgID, s32 posX, s32 posY, s32 opacity, s32 palette,
                   s32 style) {
