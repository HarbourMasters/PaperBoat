...
void decode_yay0(void *src, void *dst) {
  ...
  u32 magic = (srcBytes[0] << 24) | (srcBytes[1] << 16) | (srcBytes[2] << 8) |
              srcBytes[3];
  u32 decompSize = (srcBytes[4] << 24) | (srcBytes[5] << 16) |
                   (srcBytes[6] << 8) | srcBytes[7];

  u32 linkTableOffset = (srcBytes[8] << 24) | (srcBytes[9] << 16) |
                        (srcBytes[10] << 8) | srcBytes[11];
  u32 chunkOffset = (srcBytes[12] << 24) | (srcBytes[13] << 16) |
                    (srcBytes[14] << 8) | srcBytes[15];
  ...
  // Clamp to a sane maximum to avoid overrunning fixed buffers (e.g.
  // gMapShapeData)
