...
void nuPiReadRom(u32 romAddr, void *ramAddr, u32 len) { return; }
...
void nuGfxDisplayOn(void) { nuGfxDisplay = NU_GFX_DISPLAY_ON; }

void nuGfxDisplayOff(void) { nuGfxDisplay = NU_GFX_DISPLAY_OFF; }
...
void nuGfxPreNMIFuncSet(NUGfxPreNMIFunc func) { nuGfxPreNMIFunc = func; }

void nuGfxSwapCfbFuncSet(NUGfxSwapCfbFunc func) { nuGfxSwapCfbFunc = func; }

void nuGfxTaskEndFuncSet(NUGfxTaskEndFunc func) { nuGfxTaskEndFunc = func; }
...
void nuContDataLock(void) { return; }
...
void *fx_small_gold_sparkle(s32 arg0, f32 arg1, f32 arg2, f32 arg3, f32 arg4,
                            s32 arg5) {
  return NULL;
}
...
void osViRepeatLine(u8 repeat) { (void)repeat; }
...
void osViBlack(u8 black) { (void)black; }
...
void osSpTaskLoad(OSTask *tp) { (void)tp; }

void osSpTaskStartGo(OSTask *tp) { (void)tp; }
...
void osViSwapBuffer(void *frameBufPtr) { (void)frameBufPtr; }
...
void osContGetQuery(OSContStatus *data) { (void)data; }
...
void osMapTLB(s32 index, OSPageMask pm, void *vaddr, u32 paddr, u32 paddr_end,
              s32 asid) {
  (void)index;
  (void)pm;
  (void)vaddr;
...
s32 osPfsAllocateFile(OSPfs *pfs, u16 companyCode, u32 gameCode, u8 *gameName,
                      u8 *extName, int size, s32 *fileNo) {
  (void)pfs;
  (void)companyCode;
  (void)gameCode;
  (void)gameName;
...
s32 osPfsFindFile(OSPfs *pfs, u16 companyCode, u32 gameCode, u8 *gameName,
                  u8 *extName, s32 *fileNo) {
  (void)pfs;
  (void)companyCode;
  (void)gameCode;
  (void)gameName;
...
s32 osPfsDeleteFile(OSPfs *pfs, u16 companyCode, u32 gameCode, u8 *gameName,
                    u8 *extName) {
  (void)pfs;
  (void)companyCode;
  (void)gameCode;
...
s32 osPfsReadWriteFile(OSPfs *pfs, s32 fileNo, u8 flag, int offset, int size,
                       u8 *data) {
  (void)pfs;
  (void)fileNo;
  (void)flag;
