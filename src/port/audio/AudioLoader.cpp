...
// Size of the SBN data - we'll use a large value since we don't track actual size
static constexpr size_t kMaxSbnSize = 16 * 1024 * 1024; // 16MB should be more than enough

AudioReader* Audio_MakeReader(int32_t offset) {
    auto reader = new Ship::BinaryReader((char*)gSbnData + offset, kMaxSbnSize - offset);
