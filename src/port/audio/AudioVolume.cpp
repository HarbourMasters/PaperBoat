...
void AudioVolume_Update(void) {
    sMaster.store(ClampPercent(CVarGetInteger(CVAR_SETTING("Volume.Master"), 100)),
                  std::memory_order_relaxed);
    sMusic.store(ClampPercent(CVarGetInteger(CVAR_SETTING("Volume.MainMusic"), 100)),
                 std::memory_order_relaxed);
    sSfx.store(ClampPercent(CVarGetInteger(CVAR_SETTING("Volume.SFX"), 100)),
               std::memory_order_relaxed);
    sAmbience.store(ClampPercent(CVarGetInteger(CVAR_SETTING("Volume.Environment"), 100)),
                    std::memory_order_relaxed);
}
...
uint16_t AudioVolume_ScaleBusGain(uint8_t busID, uint16_t gain) {
    ...
    int32_t percent = (busID == 1) ? sSfx.load(std::memory_order_relaxed)
                                   : sMusic.load(std::memory_order_relaxed);
