...
extern "C" {
void guOrtho(Mtx *dest, float left, float right, float bottom, float oTop,
             float oNear, float oFar, float oScale);
}
...
struct InterpolateCtx {
  ...
  MtxF *new_replacement(Mtx *addr) { return &mtx_replacements[addr]; }
  ...
  float lerp(f32 o, f32 n) { return w * o + step * n; }
    ...
    for (auto &item : new_path->items) {
      ...
      if (item.first == Op::OpenChild) {
        if (auto it = old_path->children.find(new_op.open_child.key);
            it != old_path->children.end() &&
            new_op.open_child.idx < it->second.size()) {
          interpolate_branch(&it->second[new_op.open_child.idx],
                             &new_path->children.find(new_op.open_child.key)
                                  ->second[new_op.open_child.idx]);
        } else {
          interpolate_branch(&new_path->children.find(new_op.open_child.key)
                                  ->second[new_op.open_child.idx],
                             &new_path->children.find(new_op.open_child.key)
                                  ->second[new_op.open_child.idx]);
        }
      ...
      if (auto it = old_path->ops.find(item.first); it != old_path->ops.end()) {
        if (item.second < it->second.size()) {
          ...
          case Op::MtxFMult:
            interpolate_mtxf(&tmp_mtxf, &old_op.MtxF_mult.mf,
                             &new_op.MtxF_mult.mf);
          ...
          case Op::MtxFRotate1Coord: {
            float v = interpolate_angle(old_op.MtxF_rotate_1_coord.value,
                                        new_op.MtxF_rotate_1_coord.value);
          ...
          case Op::MtxFMultVec3fNoTranslate: {
            interpolate_vecs(&tmp_vec3f, &old_op.MtxF_vec_no_translate.src,
                             &new_op.MtxF_vec_no_translate.src);
            interpolate_vecs(&tmp_vec3f2, &old_op.MtxF_vec_no_translate.dest,
                             &new_op.MtxF_vec_no_translate.dest);
            // MtxF_MultVec3fNoTranslate(gInterpolationMtxF, &tmp_vec3f,
            // &tmp_vec3f2);
            break;
          }
          case Op::MtxFMultVec3f: {
            interpolate_vecs(&tmp_vec3f, &old_op.MtxF_vec_translate.src,
                             &new_op.MtxF_vec_translate.src);
            interpolate_vecs(&tmp_vec3f2, &old_op.MtxF_vec_translate.dest,
                             &new_op.MtxF_vec_translate.dest);
            // MtxF_MultVec3f(gInterpolationMtxF, &tmp_vec3f, &tmp_vec3f2);
            break;
          }

          case Op::MtxFMtxFToMtx:
            interpolate_mtxf(new_replacement(new_op.MtxF_mtxf_to_mtx.dest),
          ...
          case Op::MtxFToMtx: {
            ...
            if (old_op.MtxF_to_mtx.has_adjusted &&
                new_op.MtxF_to_mtx.has_adjusted) {
              interpolate_mtxf(&tmp_mtxf, &old_op.MtxF_to_mtx.src,
                               &new_op.MtxF_to_mtx.src);
              // MtxF_MtxFMtxFMult(&actor_mtx, &tmp_mtxf,
              // new_replacement(new_op.MtxF_to_mtx.dest));
            } else {
              interpolate_mtxf(new_replacement(new_op.MtxF_to_mtx.dest),
          ...
          case Op::MtxFRotateAxis: {
            lerp_vec3f(&tmp_vec3f, &old_op.MtxF_rotate_axis.axis,
                       &new_op.MtxF_rotate_axis.axis);
            auto tmp = interpolate_angle(old_op.MtxF_rotate_axis.angle,
                                         new_op.MtxF_rotate_axis.angle);
...
unordered_map<Mtx *, MtxF> FrameInterpolation_Interpolate(float step) {
  ...
  ctx.step = step;
  ctx.w = 1.0f - step;
  ctx.interpolate_branch(&previous_recording.root_path,
...
int FrameInterpolation_GetCameraEpoch(void) { return (int)camera_epoch; }
...
void FrameInterpolation_RecordMtxFTranslate(MtxF *MtxF, f32 x, f32 y, f32 z,
                                            u8 mode) {
  if (!is_recording)
    return;
...
void FrameInterpolation_RecordMtxFScale(MtxF *MtxF, f32 x, f32 y, f32 z,
                                        u8 mode) {
  if (!is_recording)
    return;
...
void FrameInterpolation_RecordMtxFMultVec3fNoTranslate(MtxF *MtxF, Vec3f src,
                                                       Vec3f dest) {
  if (!is_recording)
    return;
  append(Op::MtxFMultVec3fNoTranslate).MtxF_vec_no_translate = {MtxF, src,
                                                                dest};
}
...
void FrameInterpolation_RecordMtxFRotate1Coord(MtxF *MtxF, u32 coord, f32 value,
                                               u8 mode) {
  if (!is_recording)
    return;
