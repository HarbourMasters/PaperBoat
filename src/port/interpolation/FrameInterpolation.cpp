            ...
            if (auto it = old_path->ops.find(item.first); it != old_path->ops.end()) {
                if (item.second < it->second.size()) {
                    ...
                    switch (item.first) {
                     ...
                     // Unused on SF64
                     // case Op::MtxFPut:
                     //     interpolate_mtxf(&tmp_mtxf, &old_op.MtxF_put.src, &new_op.MtxF_put.src);
                     //     MtxF_Put(&tmp_mtxf);
                     //     break;

                        case Op::MtxFMult:
                            interpolate_mtxf(&tmp_mtxf, &old_op.MtxF_mult.mf, &new_op.MtxF_mult.mf);
                            // MtxF_Mult(gInterpolationMtxF, (MtxF*) &tmp_mtxf, new_op.MtxF_mult.mode);
                            break;

                        case Op::MtxFTranslate:
                            // MtxF_Translate(gInterpolationMtxF, lerp(old_op.MtxF_translate.x, new_op.MtxF_translate.x),
                            //                 lerp(old_op.MtxF_translate.y, new_op.MtxF_translate.y),
                            //                 lerp(old_op.MtxF_translate.z, new_op.MtxF_translate.z),
                            //                 new_op.MtxF_translate.mode);
                            break;

                        case Op::MtxFScale:
                            // MtxF_Scale(gInterpolationMtxF, lerp(old_op.MtxF_scale.x, new_op.MtxF_scale.x),
                            //              lerp(old_op.MtxF_scale.y, new_op.MtxF_scale.y),
                            //              lerp(old_op.MtxF_scale.z, new_op.MtxF_scale.z), new_op.MtxF_scale.mode);
                            break;
                        ...
                        case Op::MtxFMultVec3fNoTranslate: {
                            interpolate_vecs(&tmp_vec3f, &old_op.MtxF_vec_no_translate.src, &new_op.MtxF_vec_no_translate.src);
                            interpolate_vecs(&tmp_vec3f2, &old_op.MtxF_vec_no_translate.dest, &new_op.MtxF_vec_no_translate.dest);
                            // MtxF_MultVec3fNoTranslate(gInterpolationMtxF, &tmp_vec3f, &tmp_vec3f2);
                            break;
                        }
                        case Op::MtxFMultVec3f: {
                            interpolate_vecs(&tmp_vec3f, &old_op.MtxF_vec_translate.src, &new_op.MtxF_vec_translate.src);
                            interpolate_vecs(&tmp_vec3f2, &old_op.MtxF_vec_translate.dest, &new_op.MtxF_vec_translate.dest);
                        ...
                        case Op::MtxFMtxFToMtx:
                            interpolate_mtxf(new_replacement(new_op.MtxF_mtxf_to_mtx.dest),
                                             &old_op.MtxF_mtxf_to_mtx.src, &new_op.MtxF_mtxf_to_mtx.src);
                            break;
                        ...
                        case Op::MtxFToMtx: {
                            ...
                            if (old_op.MtxF_to_mtx.has_adjusted && new_op.MtxF_to_mtx.has_adjusted) {
                                ...
                                // MtxF_MtxFMtxFMult(&actor_mtx, &tmp_mtxf, new_replacement(new_op.MtxF_to_mtx.dest));
                            } else {
                                interpolate_mtxf(new_replacement(new_op.MtxF_to_mtx.dest), &old_op.MtxF_to_mtx.src,
                        ...
                        case Op::MtxFRotateAxis: {
                            ...
                            // MtxF_RotateAxis((MtxF*) &tmp_vec3f, tmp, 1.0f, 1.0f, 1.0f, new_op.MtxF_rotate_axis.mode);
                            break;
                        }
                    }
                }
