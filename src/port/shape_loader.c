...
static void *shape_alloc(size_t size) {
  ...
  if (sArenaPos + size > sArenaSize) {
    GameEngine_LogInfo("[Shape] Arena exhausted! pos=%zu, need=%zu, limit=%zu",
...
static Gfx *LoadDisplayListByOffset(u32 dlOffset) {
  ...
  char path[128];
  snprintf(path, sizeof(path), "__OTR__shapes/%s/dlist_%X", gCurrentShapeName,
           dlOffset);

  Gfx *dl = (Gfx *)ResourceGetDataByName(path);
  if (dl == NULL) {
    GameEngine_LogInfo("[Shape] WARNING: Could not load display list from %s",
                       path);
  }
  return dl;
}
...
static ModelNodeProperty *ConvertModelNodeProperties(u8 *base, u32 offset,
                                                     s32 numProperties) {
  if (offset == 0 || numProperties <= 0)
    return NULL;

  ModelNodeProperty *native =
      shape_alloc(sizeof(ModelNodeProperty) * numProperties);
...
static ModelGroupData *ConvertModelGroupData(u8 *base, u32 offset) {
  ...
  native->transformMatrix =
      (Mtx *)Shape_OffsetToPtr(base, raw->transformMatrixOffset);
  native->lightingGroup =
      (Lightsn *)Shape_OffsetToPtr(base, raw->lightingGroupOffset);
...
static ModelNode *ConvertModelNode(u8 *base, u32 offset) {
  ...
  native->propertyList = ConvertModelNodeProperties(
      base, raw->propertyListOffset, raw->numProperties);
...
void Shape_LoadFromRawData(ShapeFile *shapeFile, const u8 *rawData,
                           size_t rawSize, const char *shapeName) {
  ...
  shapeFile->header.vertexTable =
      (Vtx_t *)Shape_OffsetToPtr(base, rawHeader->vertexTableOffset);
  shapeFile->header.modelNames =
      ConvertNameTable(base, rawHeader->modelNamesOffset);
  shapeFile->header.colliderNames =
      ConvertNameTable(base, rawHeader->colliderNamesOffset);
  shapeFile->header.zoneNames =
      ConvertNameTable(base, rawHeader->zoneNamesOffset);
