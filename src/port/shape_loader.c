...
// Bump allocator — native structs (ModelNode, ModelGroupData, etc.) are placed in
// shapeFile->data, which is a persistent global buffer. This avoids the game's
// general_heap entirely (it gets recreated during map load, invalidating pointers).
...
static ModelDisplayData* ConvertModelDisplayData(u8* base, u32 offset) {
    ...
    if (raw->displayListOffset == 0) return NULL;
